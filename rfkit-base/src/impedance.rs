use crate::{
    math::*,
    num::{ComplexNumberType, RealScalar},
    units::{ScalarUnitValue, Scale, Unit, UnitValue, UnitValueBuilder},
    util::ApproxEq,
};
    ...
    T: RealScalar + ApproxEq<Compare = T>,
{
    ...
    pub fn diff_to_se(&mut self) -> &Self {
        if self.mode == ImpedanceMode::Se {
            ...
            self.set_z(self.z / T::from_f64(2.0));
    ...
    pub fn set_gamma(&mut self, g: Complex<T>) -> &Self {
        self.g = g;
        self.z = gamma_to_z(self.g, self.z0.into());
    ...
    pub fn set_y(&mut self, y: Complex<T>) -> &Self {
        ...
        self.g = z_to_gamma(self.z, self.z0.into());
        let (rp, cp) = z_to_rpcp(self.z, &self.freq);
        self.rp.set_val(&rp);
        self.cp.set_val(&cp);
        let (rs, cs) = z_to_rscs(self.z, &self.freq);
...
impl<T: RealScalar> Default for Impedance<T> {
    fn default() -> Self {
        Self {
            ...
            z0: 50.0.into(),
            freq: ScalarUnitValue::new_scaled(&&T::from_f64(280.0), Scale::Giga, Unit::Hz),
            rp: ScalarUnitValue::new(&&T::from_f64(50.0), Scale::Base, Unit::Ohm),
            cp: ScalarUnitValue::new_scaled(&&T::ZERO, Scale::Femto, Unit::Farad),
            rs: ScalarUnitValue::new(&&T::from_f64(50.0), Scale::Base, Unit::Ohm),
            cs: ScalarUnitValue::new_scaled(&&T::ZERO, Scale::Femto, Unit::Farad),
        }
    }
}
    ...
    T: RealScalar + ApproxEq<Compare = T>,
{
    ...
    pub fn db(mut self, db: T) -> Self {
        self.mag = Some(T::from_f64(10.0).powf(db / 20.0));
        self
    }
    ...
    pub fn r_scale(self, scale: Scale) -> Self {
        match self.rp {
            Some(mut x) => _ = x.set_scale(scale),
            None => (),
        }
        match self.rs {
            Some(mut x) => _ = x.set_scale(scale),
    ...
    pub fn r_scale_str(self, scale: &str) -> Self {
        match self.rp {
            Some(mut x) => _ = x.set_scale(Scale::from_str(scale).unwrap()),
            None => (),
        }
        match self.rs {
            Some(mut x) => _ = x.set_scale(Scale::from_str(scale).unwrap()),
    ...
    pub fn rp_val_scaled(self, res: T) -> Self {
        match self.rp {
            Some(mut x) => _ = x.set_val_scaled(&res),
            None => (),
        }
        self
    }
    ...
    pub fn cp_val_scaled(self, cap: T) -> Self {
        match self.cp {
            Some(mut x) => _ = x.set_val_scaled(&cap),
            None => (),
        }
        self
    }
    ...
    pub fn build(self) -> Result<Impedance<T>, String> {
        ...
        match self.kind {
            Some(ImpedanceType::Gamma) => {
                ...
                let z = gamma_to_z(g, self.z0.into());
            ...
            Some(ImpedanceType::Z) => {
                ...
                Ok(Impedance {
                    ...
                    z: self.ri.unwrap(),
                    g: z_to_gamma(self.ri.unwrap(), self.z0.into()),
                    rp: ScalarUnitValue::builder()
                        .val(&rp)
                        .scale(Scale::Base)
                        .unit(Unit::Ohm)
            ...
            Some(ImpedanceType::RpCp) => {
                ...
                let (rs, cs) = z_to_rscs(z, &freq);
                let g = z_to_gamma(z, self.z0.into());
                Ok(Impedance {
                    mode: self.mode,
                    y: z.recip(),
                    z,
