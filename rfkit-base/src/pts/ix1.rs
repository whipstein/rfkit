...
/// A matrix wrapper around ndarray::Array1 with ComplexFloat elements
impl<T: RealScalar> Points<Complex<T>, Ix1>
where
    Complex<T>: From<Complex64>,
{
    ...
    pub fn from_vec_c64(data: Vec<Complex64>) -> Self {
        if data.is_empty() {
            return Self::new(Array1::from_elem(0, Complex::ZERO));
        }

        let shape = Self::check_vec(&data);
        let mut matrix = Self::zeros(shape);
        for (i, &value) in data.iter().enumerate() {
            matrix[i] = Complex::from(value);
        }
    ...
    /// Create a matrix from a 1D vector of complex tuples (real, imag)
    pub fn from_vec_complex(data: Vec<(T, T)>) -> Self {
        if data.is_empty() {
            return Self::new(Array1::from_elem(0, Complex::ZERO));
        }
        ...
        let mut matrix = Self::zeros(shape);
        for (i, value) in data.iter().enumerate() {
            matrix[i] = Complex::new(value.0, value.1);
        }
    ...
    /// Create a matrix from a 1D vector of f64 values
    pub fn from_real_vec(data: Vec<T>) -> Self {
        if data.is_empty() {
            return Self::new(Array1::from_elem(0, Complex::ZERO));
        }
        ...
        let mut matrix = Self::zeros(shape);
        for (i, value) in data.iter().enumerate() {
            matrix[i] = Complex::new(*value, T::ZERO);
        }
    ...
    //         matrix[i] = Complex::new(T::from(*real), T::from(*imag));
    //     }
...
impl<T: RealScalar> PtsComplex<T, Ix1> for Points<Complex<T>, Ix1> {
    // type Size = usize;
    type Tuple<'a>
        = (T, T)
    where
