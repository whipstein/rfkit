...
impl<T: RealScalar, D: Dimension> ndarray_linalg::Norm for Points<T, D> {
    type Output = T;
...
pub trait PtsComplex<T: RealScalar, D: Dimension>: Pts<Complex<T>, D> + Sized {
    // type Size;
    type Tuple<'a>
    where
        Self: 'a;
...
pub trait MatrixComplex<T: RealScalar, D: Dimension>: Matrix<Complex<T>, D> {
...
macro_rules! impl_self_math_op(
    ($trt:ident, $operator:tt, $mth:ident, $doc:expr) => (
        impl<T: Scalar, D: Dimension> $trt for Points<T, D> {
            type Output = Self;
        ...
        impl<T: Scalar, D: Dimension> $trt<&Points<T, D>> for Points<T, D> {
            type Output = Self;
...
macro_rules! impl_math_op(
    ($trt:ident, $operator:tt, $mth:ident, $lhs:ty, $rhs:ty, $output:ty, $doc:expr) => (
        impl<D: Dimension> $trt<&Points<$rhs, D>> for Points<$lhs, D> {
        ...
        impl<D: Dimension> $trt<$rhs> for Points<$lhs, D> {
            type Output = Points<$output, D>;
        ...
        impl<D: Dimension> $trt<&Points<$lhs, D>> for Points<$rhs, D> {
            type Output = Points<$output, D>;
        ...
        impl<D: Dimension> $trt<$lhs> for Points<$rhs, D> {
            type Output = Points<$output, D>;
...
macro_rules! impl_complex_math_op(
    ($trt:ident, $operator:tt, $operator2:tt, $mth:ident, $lhs:ty, $rhs:ty, $output:ty, $doc:expr) => (
        impl<D: Dimension> $trt<&Points<$rhs, D>> for Points<Complex<$lhs>, D> {
            type Output = Points<$output, D>;
        ...
        impl<D: Dimension> $trt<$rhs> for Points<Complex<$lhs>, D> {
            type Output = Points<$output, D>;
        ...
        impl<D: Dimension> $trt<&Points<$lhs, D>> for Points<Complex<$lhs>, D> {
            type Output = Points<$output, D>;
        ...
        impl<D: Dimension> $trt<$lhs> for Points<Complex<$lhs>, D> {
            type Output = Points<$output, D>;
