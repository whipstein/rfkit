...
///     ],
/// ];
/// let freq = ArrayUnitValue::new_freq_scaled(&fdata, Scale::Giga);
/// let net = Network::new(
///     &freq,
///     &z0,
///     RFParameter::S,
    ...
    U: UnitValue<T> + Frequency<T>,
{
    ...
    pub(super) ports: Vec<PortVal>,
    pub(super) freq: U,
    pub(super) npts: usize,
    pub(super) dim: (usize, usize, usize),
    pub(super) z0: Vec<Complex<T>>,
    pub(super) param: RFParameter,
    pub(super) a: OnceLock<Points3<Complex<T>>>,
    pub(super) g: OnceLock<Points3<Complex<T>>>,
    pub(super) h: OnceLock<Points3<Complex<T>>>,
    pub(super) s_power: OnceLock<Points3<Complex<T>>>,
    pub(super) s_pseudo: OnceLock<Points3<Complex<T>>>,
    pub(super) s_traveling: OnceLock<Points3<Complex<T>>>,
    pub(super) y: OnceLock<Points3<Complex<T>>>,
    pub(super) t: OnceLock<Points3<Complex<T>>>,
    pub(super) z: OnceLock<Points3<Complex<T>>>,
}

impl<T, U> Network<T, U>
where
    T: RealScalar + 'static,
    U: UnitValue<T, Value = Array1<T>> + Frequency<T>,
{
    pub fn new(
        freq: &U,
        z0: &Vec<Complex<T>>,
        param: RFParameter,
        net: &Points3<Complex<T>>,
        name: &str,
        comments: &str,
    ) -> Result<Network<T, U>, String> {
        let nports = net.slice(s![0, .., ..]).nrows();

        Network::new_w_port_names(freq, z0, param, net, name, comments, vec![""; nports])
    }
        ...
        T: 'a,
    {
        let net_recalc = match format {
            ComplexNumberType::Db => Points::from_db(&net)?,
            ComplexNumberType::MagAng => Points::from_magang(&net)?,
            ComplexNumberType::ReIm => Points::from_reim(&net)?,
        };
    ...
    pub fn new_w_port_names(
        ...
        let mut nports = net.slice(s![0, .., ..]).nrows();
        if nports != 2
            && (param == RFParameter::A
                || param == RFParameter::G
                || param == RFParameter::H
                || param == RFParameter::T)
        {
            return Err(network_err_msg(param, nports));
        }
        ...
        let mut out = Network {
            ...
            port_names: port_names.iter().map(|s| s.to_string()).collect(),
            ports: (0..nports)
                .flat_map(|i| (0..nports).map(move |j| (i, j)))
                .collect(),
            freq: freq.clone(),
            npts: net.slice(s![.., 0, 0]).len(),
            dim: net.dim(),
            ...
            g: OnceLock::new(),
            h: OnceLock::new(),
            s_power: OnceLock::new(),
            s_pseudo: OnceLock::new(),
            s_traveling: OnceLock::new(),
            t: OnceLock::new(),
            y: OnceLock::new(),
            z: OnceLock::new(),
        };
        ...
        match param {
            ...
            RFParameter::S | RFParameter::SPower => {
                out.s_power.set(net.clone());
            }
            RFParameter::SPseudo => {
                out.s_pseudo.set(net.clone());
            }
            RFParameter::STraveling => {
                out.s_traveling.set(net.clone());
            }
            RFParameter::T => {
                out.t.set(net.clone());
            }
    ...
    pub fn builder() -> NetworkBuilder<T, U> {
        NetworkBuilder::new()
    }

    pub fn a(&self) -> &Points3<Complex<T>> {
        if self.nports != 2 {
            panic!("{}", network_err_msg(RFParameter::A, self.nports));
        }
        self.a.get_or_init(|| match self.param {
            RFParameter::A => self.a.get().unwrap().clone(),
            RFParameter::G => self.g.get().unwrap().g_to_a().unwrap(),
            RFParameter::H => self.h.get().unwrap().h_to_a().unwrap(),
            RFParameter::S | RFParameter::SPower => {
                self.s_power.get().unwrap().s_to_a(&self.z0).unwrap()
            }
            RFParameter::SPseudo => self
                .s_pseudo
                .get()
                .unwrap()
                .s_to_s(&self.z0, WaveType::Pseudo, WaveType::Power)
                .unwrap()
                .s_to_a(&self.z0)
                .unwrap(),
            RFParameter::STraveling => self
                .s_traveling
                .get()
                .unwrap()
                .s_to_s(&self.z0, WaveType::Traveling, WaveType::Power)
                .unwrap()
                .s_to_a(&self.z0)
                .unwrap(),
            RFParameter::T => self.t.get().unwrap().t_to_a(&self.z0).unwrap(),
            RFParameter::Y => self.y.get().unwrap().y_to_a().unwrap(),
            RFParameter::Z => self.z.get().unwrap().z_to_a().unwrap(),
        })
    }

    pub fn a_db(&self) -> Points3<T> {
        if self.nports != 2 {
            panic!("{}", network_err_msg(RFParameter::A, self.nports));
        }
    ...
    pub fn chop_in_half(&self) -> Result<Network<T, U>, String> {
        if self.nports() != 2 {
            return Err("Cannot chop_in_half network that is not 2 ports".into());
        } else if !self.is_reciprocal() {
            return Err("Cannot chop_in_half network that is not reciprocal".into());
        }

        let mut tmp: Vec<Complex<T>> = vec![];
        ...
        for (i, mut pt) in out.axis_iter_mut(Axis(0)).enumerate() {
            ...
            let b21 = val[[1, 0]];
            let b22 = val[[1, 1]];
            let a11 = b11 / (Complex::<T>::ONE + b12);
            tmp.push(b21 * (Complex::<T>::ONE - b11 * b22 / (Complex::<T>::ONE + b12).powi(2)));
            let a22 = b22 / (Complex::<T>::ONE + b12);
            pt.assign(&array![[a11, Complex::ZERO], [Complex::ZERO, a22]].view());
        }

        let tmp_array = sqrt_phase_unwrap(Array1::<Complex<T>>::from_vec(tmp));
    ...
    pub fn connect(&self, p1: usize, net: &Network<T, U>, p2: usize) -> Network<T, U> {
        let mut out = self.clone();
        let calc_net = self.s().connect(p1, net.s(), p2).unwrap();
    ...
    pub fn g(&self) -> &Points3<Complex<T>> {
        if self.nports != 2 {
            panic!("{}", network_err_msg(RFParameter::G, self.nports));
        }
        self.g.get_or_init(|| match self.param {
            RFParameter::A => self.a.get().unwrap().a_to_g().unwrap(),
            RFParameter::G => self.g.get().unwrap().clone(),
            RFParameter::H => self.h.get().unwrap().h_to_g().unwrap(),
            RFParameter::S | RFParameter::SPower => {
                self.s_power.get().unwrap().s_to_g(&self.z0).unwrap()
            }
            RFParameter::SPseudo => self
                .s_pseudo
                .get()
            ...
            RFParameter::STraveling => self
                ...
                .s_to_g(&self.z0)
                .unwrap(),
            RFParameter::T => self.t.get().unwrap().t_to_g(&self.z0).unwrap(),
            RFParameter::Y => self.y.get().unwrap().y_to_g().unwrap(),
            RFParameter::Z => self.z.get().unwrap().z_to_g().unwrap(),
        })
    }

    pub fn g_db(&self) -> Points3<T> {
        if self.nports != 2 {
            panic!("{}", network_err_msg(RFParameter::G, self.nports));
        }
    ...
    pub fn gd(&self, port: PortVal, scale: Scale) -> Result<Points1<T>, String> {
        ...
        for i in 0..dphi.len() {
            out.push((-dphi[i] / dw[i]).scale(scale));
        }

        Ok(Points1::from_vec(out))
    }

    pub fn h(&self) -> &Points3<Complex<T>> {
        if self.nports != 2 {
            panic!("{}", network_err_msg(RFParameter::H, self.nports));
        }
        self.h.get_or_init(|| match self.param {
            RFParameter::A => self.a.get().unwrap().a_to_h().unwrap(),
            RFParameter::G => self.g.get().unwrap().g_to_h().unwrap(),
            RFParameter::H => self.h.get().unwrap().clone(),
            RFParameter::S | RFParameter::SPower => {
                self.s_power.get().unwrap().s_to_h(&self.z0).unwrap()
            }
            RFParameter::SPseudo => self
                .s_pseudo
                .get()
            ...
            RFParameter::STraveling => self
                ...
                .s_to_h(&self.z0)
                .unwrap(),
            RFParameter::T => self.t.get().unwrap().t_to_h(&self.z0).unwrap(),
            RFParameter::Y => self.y.get().unwrap().y_to_h().unwrap(),
            RFParameter::Z => self.z.get().unwrap().z_to_h().unwrap(),
        })
    }

    pub fn h_db(&self) -> Points3<T> {
        if self.nports != 2 {
            panic!("{}", network_err_msg(RFParameter::H, self.nports));
        }
    ...
    pub fn is_reciprocal(&self) -> bool {
        for pt in self.s().outer_iter() {
            if !Points::new(pt.to_owned()).is_reciprocal() {
                return false;
            }
        }
        true
    ...
    pub fn k(&self) -> Result<PortPoints<T>, String> {
        if self.nports != 2 {
            return Err(format!(
        ...
        let mut val = Array1::<T>::zeros(s11.len());
        azip!((index i, &s11 in &s11, &s12 in &s12, &s21 in &s21, &s22 in &s22, &d in &d) {
            val[i] = (T::ONE - s11.abs().powi(2) - s22.abs().powi(2) + d.abs().powi(2)) / ((s21 * s12).abs() * 2.0);
        });

        Ok(Points(val))
    }

    pub fn max_gain(&self) -> Result<PortPoints<T>, String> {
        if self.nports != 2 {
            return Err(format!(
        ...
        let mut k = self.k()?;
        let mut max_stable_gain = self.max_stable_gain()?;
        ...
        Ok(&max_stable_gain
            / (&k + Points1::<T>::from_shape_fn(k.len(), |i| (k[i].powi(2) - T::ONE).sqrt())))
    }

    pub fn max_stable_gain(&self) -> Result<PortPoints<T>, String> {
        if self.nports != 2 {
            return Err(format!(
    ...
    pub fn mu(&self) -> Result<PortPoints<T>, String> {
        if self.nports != 2 {
            return Err(format!(
        ...
        let d_abs = Array1::from_shape_fn(self.npts, |i| d[i].abs());
        let denom_arg1 = &self.s().slice(s![.., 1, 1]) - &self.s_conj().slice(s![.., 0, 0]) * &d;
        // let denom_arg1 = &self.s().slice(s![.., 1, 1]) - &self.s_conj().slice(s![.., 0, 0]) * &d;
        let denom_arg2 = &self.s().slice(s![.., 0, 1]) * &self.s_conj().slice(s![.., 1, 0]);
        let denom =
            Array1::<T>::from_shape_fn(self.npts, |i| denom_arg1[i].abs() + denom_arg2[i].abs());
        let num = &Array1::<T>::ones(self.npts)
            - &Array1::<T>::from_shape_fn(self.npts, |i| {
    ...
    pub fn mu_prime(&self) -> Result<PortPoints<T>, String> {
        if self.nports != 2 {
            return Err(format!(
        ...
        let d = &self.s().slice(s![.., 0, 0]) * &self.s().slice(s![.., 1, 1])
            - &self.s().slice(s![.., 0, 1]) * &self.s().slice(s![.., 1, 0]);
        let d_abs = Array1::<T>::from_shape_fn(self.npts, |i| d[i].abs());
    ...
    pub fn net(&self, param: RFParameter) -> &Points3<Complex<T>> {
        match param {
            RFParameter::A => {
                if self.nports != 2 {
                    panic!(
            ...
            RFParameter::S => self.s(),
            RFParameter::SPower => self.s_power(),
            RFParameter::SPseudo => self.s_pseudo(),
            RFParameter::STraveling => self.s_traveling(),
            RFParameter::T => self.t(),
            RFParameter::Y => self.y(),
            RFParameter::Z => self.z(),
        }
    }

    pub fn net_at_freq_idx(&self, param: RFParameter, idx: usize) -> Points2<Complex<T>> {
        Points::new(self.net(param).slice(s![idx, .., ..]).to_owned())
    }

    pub fn net_at_port(&self, param: RFParameter, port: PortVal) -> PortPoints<Complex<T>> {
        Points(self.net(param).slice(s![.., port.0, port.1]).to_owned())
    }

    pub fn net_at_port_conj(&self, param: RFParameter, port: PortVal) -> PortPoints<Complex<T>> {
        Points(
            self.net_conj(param)
    ...
    pub fn net_at_port_idx(
        &self,
        param: RFParameter,
        j: usize,
        k: usize,
    ) -> PortPoints<Complex<T>> {
        Points(self.net(param).slice(s![.., j, k]).to_owned())
    }

    pub fn net_at_idx(&self, param: RFParameter, idx: usize, j: usize, k: usize) -> Complex<T> {
        self.net_at_freq_idx(param, idx)[[j, k]]
    }

    pub fn net_conj(&self, param: RFParameter) -> Points3<Complex<T>> {
        Points::from_shape_fn(self.net(param).dim().into_dimension(), |(i, j, k)| {
            self.net(param)[[i, j, k]].conj()
        })
    }

    pub fn net_db(&self, param: RFParameter) -> Points3<T> {
        self.net(param).db()
    }

    pub fn net_deg(&self, param: RFParameter) -> Points3<T> {
        self.net(param).deg()
    }

    pub fn net_im(&self, param: RFParameter) -> Points3<T> {
        self.net(param).im()
    }

    pub fn net_mag(&self, param: RFParameter) -> Points3<T> {
        self.net(param).mag()
    }

    pub fn net_rad(&self, param: RFParameter) -> Points3<T> {
        self.net(param).rad()
    }

    pub fn net_rad_at_port(&self, param: RFParameter, port: PortVal) -> PortPoints<T> {
        Points(
            self.net(param)
                .rad()
                .slice(s![.., port.0, port.1])
    ...
    pub fn net_re(&self, param: RFParameter) -> Points3<T> {
        self.net(param).re()
    }
    ...
    pub fn passivity(&self) -> Array1<T> {
        if self.nports < 2 {
            panic!(
                "{}",
                format!(
        ...
        for (_i, pt) in self.s().outer_iter().enumerate() {
            let mut acc: Array2<Complex<T>> = pt.map(|x| x.conj()).t().to_owned();
            acc = acc.dot(&pt);

            let mat = Array2::<Complex<T>>::eye(acc.nrows()) - acc;
            let eigenvalues = eig_hermitian_real(&mat);

            let mut val = eigenvalues[0];
            for &x in eigenvalues.iter() {
                if x < val {
                    val = x;
                }
            }
    ...
    pub fn ports(&self) -> &Vec<PortVal> {
        &self.ports
    }

    pub fn reciprocity(&self) -> Points3<T> {
        self.s().reciprocity().unwrap()
    }

    pub fn s(&self) -> &Points3<Complex<T>> {
        self.s_power()
    }

    pub fn s_power(&self) -> &Points3<Complex<T>> {
        self.s_power.get_or_init(|| match self.param {
            RFParameter::A => self.a.get().unwrap().a_to_s(&self.z0).unwrap(),
            RFParameter::G => self.g.get().unwrap().g_to_s(&self.z0).unwrap(),
            RFParameter::H => self.h.get().unwrap().h_to_s(&self.z0).unwrap(),
            RFParameter::S | RFParameter::SPower => self.s_power.get().unwrap().clone(),
            RFParameter::SPseudo => self
                .s_pseudo
                .get()
                .unwrap()
            ...
            RFParameter::STraveling => self
                ...
                .s_to_s(&self.z0, WaveType::Traveling, WaveType::Power)
                .unwrap(),
            RFParameter::T => self.t.get().unwrap().t_to_s().unwrap(),
            RFParameter::Y => self.y.get().unwrap().y_to_s(&self.z0).unwrap(),
            RFParameter::Z => self.z.get().unwrap().z_to_s(&self.z0).unwrap(),
        })
    }

    pub fn s_pseudo(&self) -> &Points3<Complex<T>> {
        self.s_pseudo.get_or_init(|| match self.param {
            RFParameter::A => self
                .a
                .get()
                .unwrap()
                .a_to_s(&self.z0)
                .unwrap()
                .s_to_s(&self.z0, WaveType::Power, WaveType::Pseudo)
                .unwrap(),
            RFParameter::G => self
                .g
                .get()
            ...
            RFParameter::H => self
                ...
                .s_to_s(&self.z0, WaveType::Power, WaveType::Pseudo)
                .unwrap(),
            RFParameter::S | RFParameter::SPower => self
                .s_power
                .get()
                .unwrap()
                .s_to_s(&self.z0, WaveType::Power, WaveType::Pseudo)
                .unwrap(),
            RFParameter::SPseudo => self.s_pseudo.get().unwrap().clone(),
            RFParameter::STraveling => self
                .s_traveling
                .get()
                .unwrap()
                .s_to_s(&self.z0, WaveType::Traveling, WaveType::Pseudo)
                .unwrap(),
            RFParameter::T => self
                .t
                .get()
    ...
    pub fn s_traveling(&self) -> &Points3<Complex<T>> {
        self.s_traveling.get_or_init(|| match self.param {
            RFParameter::A => self
                .a
                .get()
                .unwrap()
                .a_to_s(&self.z0)
                .unwrap()
                .s_to_s(&self.z0, WaveType::Power, WaveType::Traveling)
                .unwrap(),
            RFParameter::G => self
                .g
                .get()
            ...
            RFParameter::SPseudo => self
                ...
                .get()
                .unwrap()
                .s_to_s(&self.z0, WaveType::Pseudo, WaveType::Traveling)
                .unwrap(),
            RFParameter::STraveling => self.s_traveling.get().unwrap().clone(),
            RFParameter::T => self
                .t
                .get()
                .unwrap()
    ...
    pub fn s_conj(&self) -> Points3<Complex<T>> {
        self.s().conj()
    }

    pub fn s_db(&self) -> Points3<T> {
        self.net_db(RFParameter::S)
    }

    pub fn s_deg(&self) -> Points3<T> {
        self.net_deg(RFParameter::S)
    }

    pub fn s_im(&self) -> Points3<T> {
        self.net_im(RFParameter::S)
    }

    pub fn s_mag(&self) -> Points3<T> {
        self.net_mag(RFParameter::S)
    }

    pub fn s_rad(&self) -> Points3<T> {
        self.net_rad(RFParameter::S)
    }

    pub fn s_re(&self) -> Points3<T> {
        self.net_re(RFParameter::S)
    }

    pub fn set_net(&mut self, net: &Points3<Complex<T>>, param: RFParameter) {
        self.a = OnceLock::new();
        self.g = OnceLock::new();
        self.h = OnceLock::new();
        self.s_power = OnceLock::new();
        self.s_pseudo = OnceLock::new();
        self.s_traveling = OnceLock::new();
        self.t = OnceLock::new();
        self.y = OnceLock::new();
        self.z = OnceLock::new();
        let _ = match param {
            RFParameter::A => self.a.set(net.clone()),
            RFParameter::G => self.g.set(net.clone()),
            RFParameter::H => self.h.set(net.clone()),
            RFParameter::S | RFParameter::SPower => self.s_power.set(net.clone()),
            RFParameter::SPseudo => self.s_pseudo.set(net.clone()),
            RFParameter::STraveling => self.s_traveling.set(net.clone()),
            RFParameter::T => self.t.set(net.clone()),
            RFParameter::Y => self.y.set(net.clone()),
            RFParameter::Z => self.z.set(net.clone()),
        };
        self.param = param;
    }
    ...
    pub fn t(&self) -> &Points3<Complex<T>> {
        if self.nports != 2 {
            panic!("{}", network_err_msg(RFParameter::T, self.nports));
        }
        self.t.get_or_init(|| match self.param {
            RFParameter::A => self.a.get().unwrap().a_to_t(&self.z0).unwrap(),
            RFParameter::G => self.g.get().unwrap().g_to_t(&self.z0).unwrap(),
            RFParameter::H => self.h.get().unwrap().h_to_t(&self.z0).unwrap(),
            RFParameter::S | RFParameter::SPower => self.s_power.get().unwrap().s_to_t().unwrap(),
            RFParameter::SPseudo => self
                .s_pseudo
                .get()
                .unwrap()
            ...
            RFParameter::STraveling => self
                ...
                .s_to_t()
                .unwrap(),
            RFParameter::T => self.t.get().unwrap().clone(),
            RFParameter::Y => self.y.get().unwrap().y_to_t(&self.z0).unwrap(),
            RFParameter::Z => self.z.get().unwrap().z_to_t(&self.z0).unwrap(),
        })
    }

    pub fn t_db(&self) -> Points3<T> {
        if self.nports != 2 {
            panic!("{}", network_err_msg(RFParameter::T, self.nports));
        }
    ...
    pub fn y(&self) -> &Points3<Complex<T>> {
        self.y.get_or_init(|| match self.param {
            RFParameter::A => self.a.get().unwrap().a_to_y().unwrap(),
            RFParameter::G => self.g.get().unwrap().g_to_y().unwrap(),
            RFParameter::H => self.h.get().unwrap().h_to_y().unwrap(),
            RFParameter::S | RFParameter::SPower => {
                self.s_power.get().unwrap().s_to_y(&self.z0).unwrap()
            }
            RFParameter::SPseudo => self
                .s_pseudo
                .get()
            ...
            RFParameter::STraveling => self
                ...
                .s_to_y(&self.z0)
                .unwrap(),
            RFParameter::T => self.t.get().unwrap().t_to_y(&self.z0).unwrap(),
            RFParameter::Y => self.y.get().unwrap().clone(),
            RFParameter::Z => self.z.get().unwrap().z_to_y().unwrap(),
        })
    }

    pub fn y_db(&self) -> Points3<T> {
        self.net_db(RFParameter::Y)
    }

    pub fn y_deg(&self) -> Points3<T> {
        self.net_deg(RFParameter::Y)
    }

    pub fn y_im(&self) -> Points3<T> {
        self.net_im(RFParameter::Y)
    }

    pub fn y_mag(&self) -> Points3<T> {
        self.net_mag(RFParameter::Y)
    }

    pub fn y_rad(&self) -> Points3<T> {
        self.net_rad(RFParameter::Y)
    }

    pub fn y_re(&self) -> Points3<T> {
        self.net_re(RFParameter::Y)
    }

    pub fn z(&self) -> &Points3<Complex<T>> {
        self.z.get_or_init(|| match self.param {
            RFParameter::A => self.a.get().unwrap().a_to_z().unwrap(),
            RFParameter::G => self.g.get().unwrap().g_to_z().unwrap(),
            RFParameter::H => self.h.get().unwrap().h_to_z().unwrap(),
            RFParameter::S | RFParameter::SPower => {
                self.s_power.get().unwrap().s_to_z(&self.z0).unwrap()
            }
            RFParameter::SPseudo => self
                .s_pseudo
                .get()
            ...
            RFParameter::STraveling => self
                ...
                .s_to_z(&self.z0)
                .unwrap(),
            RFParameter::T => self.t.get().unwrap().t_to_z(&self.z0).unwrap(),
            RFParameter::Y => self.y.get().unwrap().y_to_z().unwrap(),
            RFParameter::Z => self.z.get().unwrap().clone(),
        })
    }

    pub fn z_db(&self) -> Points3<T> {
        self.net_db(RFParameter::Z)
    }

    pub fn z_deg(&self) -> Points3<T> {
        self.net_deg(RFParameter::Z)
    }

    pub fn z_im(&self) -> Points3<T> {
        self.net_im(RFParameter::Z)
    }

    pub fn z_mag(&self) -> Points3<T> {
        self.net_mag(RFParameter::Z)
    }

    pub fn z_rad(&self) -> Points3<T> {
        self.net_rad(RFParameter::Z)
    }

    pub fn z_re(&self) -> Points3<T> {
    ...
    pub fn z0_at_port_idx(&self, idx: usize) -> &Complex<T> {
        &self.z0[idx]
    }

    pub fn z0_to_vector(&self) -> Vec<Complex<T>> {
        let mut out: Vec<Complex<T>> = vec![];
...
impl<T, U> fmt::Display for Network<T, U>
where
    T: RealScalar,
    U: UnitValue<T> + Frequency<T>,
{
...
impl<T, U> fmt::Debug for Network<T, U>
where
    T: RealScalar + 'static,
    U: UnitValue<T, Value = Array1<T>> + Frequency<T> + fmt::Debug,
{
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
...
mod network_network_tests {
    ...
    fn compare_2ports(
        ...
        for (param, exemplar) in exemplars {
            ...
            match param {
                ...
                RFParameter::G => {
                    ...
                    comp_pts_ix3(&exemplar, calc.g(), margin, "g()");
                    new_net.set_net(&x, param);
                    comp_pts_ix3(&x, new_net.g(), margin, "g(x)");
                    new_net.set_net(calc.g(), param);
                    comp_pts_ix3(&exemplar, new_net.g(), margin, "g(new_net)");
                }
                RFParameter::H => {
                    comp_pts_ix3(
                        &exemplar,
                    ...
                    comp_pts_ix3(&exemplar, calc.h(), margin, "h()");
                    new_net.set_net(&x, param);
                    comp_pts_ix3(&x, new_net.h(), margin, "h(x)");
                    new_net.set_net(calc.h(), param);
                    comp_pts_ix3(&exemplar, new_net.h(), margin, "h(new_net)");
                }
                RFParameter::S => {
                    comp_pts_ix3(
                        &exemplar,
                    ...
                    comp_pts_ix3(&exemplar, calc.s(), margin, "s()");
                    new_net.set_net(&x, param);
                    comp_pts_ix3(&x, new_net.s(), margin, "s(x)");
                    new_net.set_net(calc.s(), param);
                    comp_pts_ix3(&exemplar, new_net.s(), margin, "s(new_net)");
                }
                RFParameter::SPower => {
                    comp_pts_ix3(
                        &exemplar,
                    ...
                    comp_pts_ix3(&exemplar, calc.s_power(), margin, "s_power()");
                    new_net.set_net(&x, param);
                    comp_pts_ix3(&x, new_net.s_power(), margin, "s_power(x)");
                    new_net.set_net(calc.s_power(), param);
                    comp_pts_ix3(&exemplar, new_net.s_power(), margin, "s_power(new_net)");
                }
                RFParameter::SPseudo => {
                    comp_pts_ix3(
                        &exemplar,
                    ...
                    comp_pts_ix3(&exemplar, calc.s_pseudo(), margin, "s_pseudo()");
                    new_net.set_net(&x, param);
                    comp_pts_ix3(&x, new_net.s_pseudo(), margin, "s_pseudo(x)");
                    new_net.set_net(calc.s_pseudo(), param);
                    comp_pts_ix3(&exemplar, new_net.s_pseudo(), margin, "s_pseudo(new_net)");
                }
                RFParameter::STraveling => {
                    comp_pts_ix3(
                        &exemplar,
                    ...
                    comp_pts_ix3(&exemplar, calc.s_traveling(), margin, "s_traveling()");
                    new_net.set_net(&x, param);
                ...
                RFParameter::T => {
                    ...
                    comp_pts_ix3(&exemplar, calc.t(), margin, "t()");
                    new_net.set_net(&x, param);
                    comp_pts_ix3(&x, new_net.t(), margin, "t(x)");
                    new_net.set_net(calc.t(), param);
                    comp_pts_ix3(&exemplar, new_net.t(), margin, "t(new_net)");
                }
                RFParameter::Y => {
                    comp_pts_ix3(
                        &exemplar,
                    ...
                    comp_pts_ix3(&exemplar, calc.y(), margin, "y()");
                    new_net.set_net(&x, param);
                    comp_pts_ix3(&x, new_net.y(), margin, "y(x)");
                    new_net.set_net(calc.y(), param);
                    comp_pts_ix3(&exemplar, new_net.y(), margin, "y(new_net)");
                }
                RFParameter::Z => {
                    comp_pts_ix3(
                        &exemplar,
                    ...
                    comp_pts_ix3(&exemplar, calc.z(), margin, "z()");
                    new_net.set_net(&x, param);
                    comp_pts_ix3(&x, new_net.z(), margin, "z(x)");
                    new_net.set_net(calc.z(), param);
                    comp_pts_ix3(&exemplar, new_net.z(), margin, "z(new_net)");
                }
            }
        }
    }

    mod network_network_parameter_tests {
        use super::*;

        #[test]
        fn network_reciprocity() {
            let calc = Network::new(
                &ArrayUnitValue::new_freq_scaled(&array![1.0], Scale::Giga),
                &vec![c64(50.0, 0.0), c64(50.0, 0.0)],
                RFParameter::S,
                &points![[
                    [c64(0.958, -0.263), c64(-0.846, 0.158)],
            ...
            let exemplar = points![[[0.0, 0.860811245279707], [0.860811245279707, 0.0],]];
        ...
        fn network_chop_in_half() {
            let base = Network::new(
                &ArrayUnitValue::builder()
                    .freq_scaled(&array![1.0], Scale::Giga)
                    .build()
                    .unwrap(),
                &vec![c64(50.0, 0.0), c64(50.0, 0.0)],
                RFParameter::S,
            ...
            let calc = base.chop_in_half().unwrap();

            comp_pts_ix3(&exemplar, calc.s(), NumMargin::default(), "chop_in_half");
        }
        ...
        fn network_connect() {
            ...
            let net = Network::new(
                &ArrayUnitValue::builder()
                    .freq_scaled(&array![1.0, 2.0, 3.0], Scale::Giga)
                    .build()
                    .unwrap(),
                &vec![c64(50.0, 0.0), c64(50.0, 0.0)],
                param,
            ...
            comp_pts_ix3(&exemplar, test.s(), margin, "connect(1, net, 0)");
            ...
            // let net = Network::new(
            //     &ArrayUnitValue::builder().freq_scaled(&array![1.0, 2.0, 3.0], Scale::Giga),
            ...
            //     ulps: 10,
            // };
            // comp_pts_ix3(&exemplar, &test.s(), margin, "connect(1, net2, 0)");
        }
        ...
        fn network_gd() {
            ...
            let exemplar: Points1<f64> =
                points![-90.132597176469145, 49.955810487903721, 190.04421815227662];
            let calc = Network::new(
                &ArrayUnitValue::builder()
                    .freq_scaled(&array![1.0, 2.0, 3.0], Scale::Giga)
                    .build()
            ...
            comp_pts_ix1(
                &exemplar,
                &calc.gd((1, 0), Scale::Pico).unwrap(),
                margin,
                "group_delay1((1,0), Scale::Pico)",
            );

            let exemplar: Points1<f64> = points![
                22.9888888888889,
                22.98888888888887,
                22.894444444444417,
                22.638888888888893,
            ...
            let calc = Network::new(
                &ArrayUnitValue::builder()
                    .freq_scaled(&array![0.5, 1.0, 1.5, 2.0], Scale::Giga)
                    .build()
                    .unwrap(),
                &vec![c64(50.0, 0.0), c64(50.0, 0.0)],
                RFParameter::S,
        ...
        fn network_k() {
            ...
            .unwrap();
            comp_array_f64(exemplar.view(), calc.k().unwrap().view(), margin, "k(1)");
            ...
            .unwrap();
            comp_array_f64(exemplar.view(), calc.k().unwrap().view(), margin, "k(2)");
            ...
            comp_array_f64(exemplar.view(), calc.k().unwrap().view(), margin, "k(3)");
            ...
            .unwrap();
            comp_array_f64(exemplar.view(), calc.k().unwrap().view(), margin, "k(4)");
        }
        ...
        fn network_max_gain() {
            ...
            let exemplar_max_gain: Points1<f64> = points![
                2.598182739029478e-02,
                2.269447098311342e-05,
                7.858595949885737e-03,
            ];
            let exemplar_max_stable_gain: Points1<f64> = points![
            ...
            comp_array_f64(
                ...
                calc.max_gain().unwrap().view(),
                margin,
                "max_gain(1)",
            );
            comp_pts_ix1(
                &exemplar_max_stable_gain,
                &calc.max_stable_gain().unwrap(),
                margin,
                "max_stable_gain(1)",
            );
        ...
        fn network_mu() {
            ...
            let exemplar_mu: Points1<f64> =
                points![0.321707950574174, -0.002550951441449, -1.449447552621264];
            let exemplar_mu_prime: Points1<f64> =
                points![0.976861302098624, -0.445955657662617, 0.0408012371672];
            let calc = Network::new(
                &ArrayUnitValue::builder()
                    .freq_scaled(&array![1.0, 2.0, 3.0], Scale::Giga)
                    .build()
            ...
            .unwrap();
            comp_pts_ix1(&exemplar_mu, &calc.mu().unwrap(), margin, "mu(1)");
            comp_pts_ix1(
                &exemplar_mu_prime,
                &calc.mu_prime().unwrap(),
                margin,
                "mu_prime(1)",
            );

            // test.s2p
            let exemplar_mu: Points1<f64> = array![
                0.033183769882311,
                0.041130227236983,
                0.037333083239591,
                0.054219705626116,
            ...
            .into();
            let exemplar_mu_prime: Points1<f64> = array![
                0.888443470698538,
                0.796693849590944,
                0.717478768636478,
                0.655211604332342,
            ...
            .unwrap();
            comp_pts_ix1(&exemplar_mu, &calc.mu().unwrap(), margin, "mu(2)");
            comp_pts_ix1(
                &exemplar_mu_prime,
                &calc.mu_prime().unwrap(),
                margin,
            ...
            // test_2.s2p
            let exemplar_mu: Points1<f64> = points![
                0.252051215740252,
                0.639380809343797,
                0.660662373486303,
                0.69037752619849,
            ...
            let exemplar_mu_prime: Points1<f64> = points![
                0.999513861245046,
                0.967558564020585,
                0.74956916752738,
                0.728865728310883,
            ...
            comp_pts_ix1(&exemplar_mu, &calc.mu().unwrap(), margin, "mu(3)");
            comp_pts_ix1(
                &exemplar_mu_prime,
                &calc.mu_prime().unwrap(),
                margin,
            ...
            .unwrap();
            comp_pts_ix1(&exemplar_mu, &calc.mu().unwrap(), margin, "mu(4)");
            comp_pts_ix1(
                &exemplar_mu_prime,
                &calc.mu_prime().unwrap(),
                margin,
        ...
        fn network_ports() {
            ...
            let exemplar_ne = vec![(0, 0), (0, 1), (1, 0), (1, 1)];
            ...
            let mut calc = Network::new(
                &ArrayUnitValue::builder()
                    ...
                    .build()
                    .unwrap(),
                &vec![c64(50.0, 0.0), c64(50.0, 0.0), c64(50.0, 0.0)],
                RFParameter::S,
                &points![[
    ...
    mod network_network_type_tests {
        use super::*;
        ...
        fn network_s_db() {
            ...
            comp_pts_ix3(&exemplar_s, calc.s(), NumMargin::default(), "s()");
            comp_pts_ix3(&exemplar_s_db, &calc.s_db(), NumMargin::default(), "s_db()");
        }
        ...
        fn network_s_im() {
            ...
            comp_pts_ix3(&exemplar_s, calc.s(), NumMargin::default(), "s()");
            comp_pts_ix3(&exemplar_s_im, &calc.s_im(), NumMargin::default(), "s_im()");
        }
        ...
        fn network_s_re() {
            ...
            comp_pts_ix3(&exemplar_s, calc.s(), NumMargin::default(), "s()");
            comp_pts_ix3(&exemplar_s_re, &calc.s_re(), NumMargin::default(), "s_re()");
        }
    }

    mod network_network_a_tests {
        use super::*;
    ...
    mod network_network_s_tests {
        ...
        fn network_s_to_1port() {
            ...
            let margin = MARGIN;
            comp_pts_ix3(&exemplar_s, calc.net(RFParameter::S), margin, "net(S)");
            comp_pts_ix3(&exemplar_s, calc.s(), margin, "s()");
            new_net.set_net(calc.s(), RFParameter::S);
            comp_pts_ix3(&exemplar_s, new_net.s(), margin, "s(new_net)");

            comp_pts_ix3(&exemplar_y, calc.net(RFParameter::Y), margin, "net(Y)");
            comp_pts_ix3(&exemplar_y, calc.y(), margin, "y()");
            new_net.set_net(calc.y(), RFParameter::Y);
            comp_pts_ix3(&exemplar_y, new_net.y(), margin, "y(new_net)");

            comp_pts_ix3(&exemplar_z, calc.net(RFParameter::Z), margin, "net(Z)");
            comp_pts_ix3(&exemplar_z, calc.z(), margin, "z()");
            new_net.set_net(calc.z(), RFParameter::Z);
            comp_pts_ix3(&exemplar_z, new_net.z(), margin, "z(new_net)");
        }
    ...
    mod network_network_y_tests {
        ...
        fn network_y_to_1port() {
            ...
            let exemplar_s = points![[[c64(-0.9618584453026511, 0.010256880250923072)]]];

            let exemplar_y = exemplar;

            let exemplar_z = points![[[c64(0.9706839268724422, 0.26648212188669346)]]];
    ...
    mod network_network_z_tests {
        ...
        fn network_z_to_1port() {
            ...
            let exemplar_s = points![[[c64(-0.9623481369389654, -0.010127900624336668)]]];

            let exemplar_y = points![[[c64(0.9706839268724422, 0.26648212188669346)]]];
        ...
        fn network_z_to_2port() {
            ...
            let margin = MARGIN;
            comp_pts_ix3(&exemplar_a, calc.net(RFParameter::A), margin, "net(A)");
            comp_pts_ix3(&exemplar_a, calc.a(), margin, "a()");
            new_net.set_net(calc.a(), RFParameter::A);
            comp_pts_ix3(&exemplar_a, new_net.a(), margin, "a(new_net)");

            comp_pts_ix3(&exemplar_g, calc.net(RFParameter::G), margin, "net(G)");
            comp_pts_ix3(&exemplar_g, calc.g(), margin, "g()");
            new_net.set_net(calc.g(), RFParameter::G);
            comp_pts_ix3(&exemplar_g, new_net.g(), margin, "g(new_net)");

            comp_pts_ix3(&exemplar_h, calc.net(RFParameter::H), margin, "net(H)");
            comp_pts_ix3(&exemplar_h, calc.h(), margin, "h()");
            new_net.set_net(calc.h(), RFParameter::H);
            comp_pts_ix3(&exemplar_h, new_net.h(), margin, "h(new_net)");
            ...
            comp_pts_ix3(&exemplar_t, calc.net(RFParameter::T), margin, "net(T)");
            comp_pts_ix3(&exemplar_t, calc.t(), margin, "t()");
            new_net.set_net(calc.t(), RFParameter::T);
            comp_pts_ix3(&exemplar_t, new_net.t(), margin, "t(new_net)");
