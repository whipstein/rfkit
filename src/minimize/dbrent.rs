...
use crate::{error::MinimizerError, minimize::ObjDerFn, num::RealScalar};
use std::fmt;
    ...
    T: RealScalar,
{
    ...
    pub fn minimize(
        ...
        while eval_b.1.abs() > tol && (bx - ax).abs() > tol && self.iters < max_iter {
            ...
            let condition1 = !(((3.0.into() * ax + bx) / 4.0.into())..=bx).contains(&s);
            let condition2 = mflag && (s - bx).abs() >= (bx - eval_c.0).abs() / 2.0.into();
            let val = eval_c.0 - eval_d.0;
            let condition3 = !mflag && (s - bx).abs() >= val.abs() / 2.0.into();
    ...
    pub fn newton_with_bracket(
        ...
        while self.iters < max_iter {
            ...
            if dfx.abs() < tol {
                ...
                x = (bracket_a + bracket_b) / 2.0.into();
...
mod minimize_myfloat_dbrent_tests {
    ...
    mod challenging_numerical_cases {
        ...
        fn test_nearly_flat_function() {
            ...
            let expected_root = TwoFloat::from_f64(0.1).pow(TwoFloat::from_f64(1.0) / 7.0);
        ...
        fn test_steep_function() {
            ...
            let f = |x: &TwoFloat| x.pow(TwoFloat::from_f64(1.0) / 7.0) - 2.0;
            let df = |x: &TwoFloat| {
                (TwoFloat::from_f64(1.0) / 7.0) * x.pow(TwoFloat::from_f64(-6.0) / 7.0)
            };
            let objective = SingleDimDerFn::new(f, df);
            let mut dbrent = DBrent::new(objective);
