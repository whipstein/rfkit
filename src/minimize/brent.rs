    ...
    T: RealScalar,
{
    ...
    pub fn line_search(
        ...
        let mut func = F1dim::new_boxed(self.f);
        ...
        while fb > fc && evaluations < max_evaluations / 3 {
            ...
            let (new_u, new_fu) = if (bx - u) * (u - cx) > T::zero() {
                ...
                if fu < fc {
                    ...
                    let u_new = cx + 1.618034.into() * (cx - bx);
                ...
                evaluations += 1;
                if fu < fc {
                    let u_new = u + 1.618034.into() * (u - cx);
                    (u_new, func.eval(point, direction, &u_new)?)
                } else {
                    (u, fu)
                }
        ...
        while evaluations < max_evaluations {
            let xm = 0.5.into() * (a + b);
            let tol1_x = tol1 * x.abs() + ZEPS.into();
            let tol2_x = 2.0.into() * tol1_x;

            if (x - xm).abs() <= tol2_x - 0.5.into() * (b - a) {
                break;
            }
                ...
                if q_final != T::zero() {
                        ...
                        && p_over_q < (b - x)
                    {
                        ...
                        if u - a < tol2_x || b - u < tol2_x {
                            d = if xm - x > T::zero() { tol1_x } else { -tol1_x };
                        }
                        use_golden = false;
                    }
                }
            ...
            u = x + if d.abs() >= tol1_x {
                ...
                if d > T::zero() { tol1_x } else { -tol1_x }
            };
            let fu = func.eval(point, direction, &u)?;
            evaluations += 1;
    ...
    pub fn minimize(
        ...
        while fb.abs() > tol && (bx - ax).abs() > tol && self.iters < max_iter {
            ...
            let condition2 = mflag && (s - bx).abs() >= (bx - c).abs() / 2.0.into();
            let condition3 = !mflag && (s - bx).abs() >= (c - d).abs() / 2.0.into();
    ...
    pub fn find_all_roots(
        ...
        let dx = (b - a) / n_sub.into();

        for i in 0..n_sub {
            let x1 = a + (i as f64).into() * dx;
            let x2 = a + ((i + 1) as f64).into() * dx;
            ...
            if f1 * f2 < T::zero() {
                if let Ok(result) = self.minimize(x1, x2, Some(tol), None) {
                    ...
                    let is_unique = roots.iter().all(|&existing_root| {
                        (result.xmin - existing_root).abs() > tol * 10.0.into()
                    });
            ...
            if f1.abs() < tol {
                let is_unique = roots
                    .iter()
                    .all(|&existing_root| (x1 - existing_root).abs() > tol * 10.0.into());
                if is_unique {
                    roots.push(x1);
                }
            }
        ...
        if self.f.call_scalar(&b).abs() < tol {
            let is_unique = roots
                .iter()
                .all(|&existing_root| (b - existing_root).abs() > tol * 10.0.into());
            if is_unique {
                roots.push(b);
            }
        }
...
mod minimize_brent_tests {
    use super::*;
    use crate::{minimize::SingleDimFn, pts::Pts};
    ...
    mod find_all_roots_tests {
        ...
        fn test_find_all_roots_polynomial() {
            ...
            assert!(
                roots
                    .iter()
                    .any(|r| approx_eq!(f64, r.into(), -2.0, TIGHT_MARGIN))
            );
            assert!(
                roots
                    .iter()
                    .any(|r| approx_eq!(f64, r.into(), 1.0, TIGHT_MARGIN))
            );
            assert!(
                roots
                    .iter()
                    .any(|r| approx_eq!(f64, r.into(), 3.0, TIGHT_MARGIN))
            );
        }
        ...
        fn test_find_all_roots_trigonometric() {
            ...
            assert!(
                roots
                    .iter()
                    .any(|r| approx_eq!(f64, r.into(), PI, TIGHT_MARGIN))
            );
        }
    ...
    mod bracket_tests {
        ...
        fn test_minimize_bracket() {
            ...
            match result {
                Ok(res) => {
                    assert!(
                        approx_eq!(f64, res.xmin.into(), 3.0, LOOSE_MARGIN)
