...
use crate::minimize::{
    ObjGradFn,
    myfloat::{LineSearchResult, WolfeParams, dot_1d_1d, dot_2d_1d, norm_1d},
};
...
pub struct QuasiNewtonResult {
    ...
    pub final_hessian_approximation: Array2<MyFloat>,
...
impl QuasiNewton {
    ...
    fn bfgs_optimization(
        ...
        while iterations < max_iters && grad_norm > *tol {
            ...
            if sy > 1e-8 * norm_1d(&s) * norm_1d(&y) {
    ...
    fn dfp_optimization(
        ...
        while iterations < max_iters && grad_norm > *tol {
            ...
            let search_direction: Array1<MyFloat> = search_direction.iter().map(|x| -x).collect();
            ...
            if sy > 1e-14 {
                ...
                if yhy > 1e-14 {
                    let hyhy =
                        Array2::from_shape_fn((hy.len(), hy.len()), |(i, j)| &hy[i] * &hy[j]);
    ...
    fn lbfgs_optimization(
        ...
        // L-BFGS storage
        let mut s_history: Array2<MyFloat> = Array2::zeros((m, n));
        let mut y_history: Array2<MyFloat> = Array2::zeros((m, n));
        let mut rho_history: Array1<MyFloat> = Array1::zeros(m);
    ...
    fn lbfgs_two_loop_recursion(
        &mut self,
        grad: &Array1<MyFloat>,
        s_history: &Array2<MyFloat>,
        y_history: &Array2<MyFloat>,
        rho_history: &Array1<MyFloat>,
    ) -> Array1<MyFloat> {
        ...
        // First loop (backward)
        for i in (0..m).rev() {
            alpha[i] = &rho_history[i] * dot_1d_1d(&s_history.row(i).to_owned(), &q);
            for j in 0..n {
        ...
        // Second loop (forward)
        for i in 0..m {
            let beta = &rho_history[i] * dot_1d_1d(&y_history.row(i).to_owned(), &q);
            for j in 0..n {
...
#[cfg(test)]
mod minimize_myfloat_quasinewton_tests {
    use super::*;
    use crate::minimize::myfloat::{MultiDimGradFn, MultiDimNumGradFn, objective::GF1dim};
