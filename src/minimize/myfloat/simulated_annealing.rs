...
pub struct AnnealingResult<T> {
    pub best_solution: T,
    pub best_energy: MyFloat,
    pub final_solution: T,
    pub final_energy: MyFloat,
...
pub trait Annealable: Clone {
    ...
    fn acceptance_probability(
        &self,
        new_energy: &MyFloat,
        current_energy: &MyFloat,
        temperature: &MyFloat,
    ) -> MyFloat {
        if new_energy < current_energy {
            1.0.into() // Always accept better solutions
        } else {
            MyFloat::exp(&(-(new_energy - current_energy) / temperature))
        }
    }
}
...
pub struct SimAnnealing {
    initial_temp: MyFloat, // Initial temperature
    final_temp: MyFloat,   // Final temperature (stopping criterion)
    alpha: MyFloat,        // Temperature reduction factor (0 < alpha < 1)
...
impl SimAnnealing {
    pub fn new(
        initial_temp: Option<MyFloat>,
        final_temp: Option<MyFloat>,
    ...
    pub fn optimize<T: Annealable>(&mut self, initial_solution: T) -> AnnealingResult<T> {
        ...
        let mut best_energy = current_energy.clone();

        let mut temperature = self.initial_temp.clone();
        ...
        while temperature > self.final_temp && total_iterations < self.max_iters {
            for _ in 0..self.iters_per_temp {
                ...
                if acceptance_prob > self.rng.random::<f64>() {
                    ...
                    current_energy = new_energy.clone();
                    accepted_moves += 1;

                    // Update best solution if necessary
                    if new_energy < best_energy {
                        best_solution = current_solution.clone();
                        best_energy = new_energy.clone();
                    }
                }
        ...
        AnnealingResult {
            ...
            final_energy: current_energy.clone(),
...
pub struct TSPSolution {
    ...
    pub distances: Vec<Vec<MyFloat>>, // Distance matrix
}

impl TSPSolution {
    pub fn new(distances: &Vec<Vec<MyFloat>>) -> Self {
        let n = distances.len();
    ...
    fn total_distance(&self) -> MyFloat {
        let n = self.cities.len();
        let mut total = MyFloat::new(0.0);
...
pub struct FunctionOptimization {
    pub x: Vec<MyFloat>,
    pub step_size: MyFloat,
    pub function: TestFunction,
}

impl FunctionOptimization {
    pub fn new(initial_x: &Vec<MyFloat>, step_size: &MyFloat, function: TestFunction) -> Self {
        Self {
            x: initial_x.clone(),
    ...
    // Highly multimodal
    fn ackley(&self) -> MyFloat {
        let n = self.x.len() as f64;
        let sum_sq = self.x.iter().map(|xi| xi * xi).sum::<MyFloat>();
        let sum_cos = self
            .x
            .iter()
    ...
    fn goldstein_price(&self) -> MyFloat {
        if self.x.len() != 2 {
            return f64::INFINITY.into();
        }
        let x = self.x[0].clone();
    ...
    fn levy(&self) -> MyFloat {
        let n = self.x.len();
        if n == 0 {
            return 0.0.into();
        }

        let w: Vec<MyFloat> = self.x.iter().map(|xi| 1.0 + (xi - 1.0) / 4.0).collect();
    ...
    fn neighbor(&self, rng: &mut impl Rng) -> Self {
        ...
        for i in 0..new_solution.x.len() {
            let perturbation = rng
                .random_range(-self.step_size.to_f64()..self.step_size.to_f64())
                .into();
...
#[cfg(test)]
mod minimize_myfloat_simannealing_tests {
    use super::*;
    ...
    fn test_function_optimization() {
        ...
        let initial =
            FunctionOptimization::new_rosenbrock(&vec![0.0.into(), 0.0.into()], &0.05.into());
        ...
        for _ in 0..5 {
            ...
            let init =
                FunctionOptimization::new_rosenbrock(&vec![0.5.into(), 0.5.into()], &0.02.into());
    ...
    fn test_sphere_function() {
        ...
        loop {
            iter += 1;
            let initial = FunctionOptimization::new(
                &vec![5.0.into(), MyFloat::new(-3.0), 2.0.into()],
                &0.1.into(),
                TestFunction::Sphere,
            );
    ...
    fn test_levy_function() {
        ...
        loop {
            ...
            let initial = FunctionOptimization::new(
                ...
                &0.05.into(),
                TestFunction::Levy,
            ); // Better starting point and smaller steps
    ...
    fn benchmark_all_functions() {
        let functions = [
            (
                TestFunction::Sphere,
                vec![MyFloat::new(2.0), MyFloat::new(-1.0)],
                "Easy - convex",
            ),
            (
            ...
            (
                TestFunction::Levy,
                vec![2.0.into(), MyFloat::new(-1.0), 1.5.into()],
        ...
        for (func, initial, description) in functions.iter() {
            ...
            let problem = FunctionOptimization::new(&initial, &0.1.into(), func.clone());
    ...
    fn test_tsp() {
        ...
        let distances = vec![
            vec![MyFloat::new(0.0), 2.0.into(), 9.0.into(), 10.0.into()],
            vec![1.0.into(), 0.0.into(), 6.0.into(), 4.0.into()],
            vec![15.0.into(), 7.0.into(), 0.0.into(), 8.0.into()],
            vec![6.0.into(), 3.0.into(), 12.0.into(), 0.0.into()],
        ];
    ...
    fn run_trials<T: Annealable>(
        ...
        trials: usize,
        target_energy: &MyFloat,
    ) -> (Vec<MyFloat>, MyFloat, usize) {
        ...
        for _ in 0..trials {
            ...
            let result = optimizer.optimize(problem);
            energies.push(result.best_energy.clone());
            if result.best_energy <= *target_energy {
                successes += 1;
            }
        }

        let avg_energy = energies.iter().sum::<MyFloat>() / energies.len() as f64;
    ...
    fn test_optimizer_construction() {
        ...
        let mut custom_opt = SimAnnealing::new(
            Some(200.0.into()),
            Some(0.01.into()),
            Some(0.9.into()),
            Some(50),
        ...
        let problem =
            FunctionOptimization::new(&vec![0.0.into()], &0.1.into(), TestFunction::Sphere);
    ...
    fn test_optimizer_edge_cases() {
        ...
        let mut low_temp_opt = SimAnnealing::new(
            Some(0.001.into()),
            Some(0.0001.into()),
            Some(0.95.into()),
            Some(10),
        ...
        let problem =
            FunctionOptimization::new(&vec![1.0.into()], &0.1.into(), TestFunction::Sphere);
        let result = low_temp_opt.optimize(problem);
        assert!(result.iterations <= 100);

        // Test with alpha close to 1 (slow cooling)
        let mut slow_cool_opt = SimAnnealing::new(
            Some(10.0.into()),
            Some(0.1.into()),
            Some(0.999.into()),
            Some(5),
    ...
    fn test_max_iterations_respected() {
        let mut optimizer = SimAnnealing::new(
            Some(1000.0.into()), // High temp
            Some(0.001.into()),  // Low final temp (would take many iterations)
            Some(0.999.into()),  // Slow cooling
            Some(10),
            Some(100), // Low max iterations
        );

        let problem = FunctionOptimization::new(
            &vec![5.0.into(), MyFloat::new(-3.0)],
            &0.1.into(),
            TestFunction::Sphere,
        );
        let result = optimizer.optimize(problem);
    ...
    fn test_convergence_properties() {
        ...
        let (energies, avg_energy, successes) = run_trials(
            ...
            trials,
            &1.0.into(), // More lenient threshold for sphere function
        );
    ...
    fn test_statistical_properties() {
        ...
        let functions_to_test = vec![
            (
                TestFunction::Sphere,
                vec![MyFloat::new(3.0), MyFloat::new(-2.0)],
                MyFloat::new(1.0),
        ...
        for (func, initial, threshold) in functions_to_test {
            let (energies, avg_energy, successes) = run_trials(
                || FunctionOptimization::new(&initial, &0.1.into(), func.clone()),
                || {
                    SimAnnealing::new(
                        Some(100.0.into()),
                        Some(0.001.into()),
    ...
    fn test_parameter_sensitivity() {
        ...
        let temps = vec![MyFloat::new(1.0), 10.0.into(), 100.0.into(), 1000.0.into()];
        ...
        let alphas = vec![MyFloat::new(0.9), 0.95.into(), 0.99.into(), 0.995.into()];
    ...
    fn test_function_properties() {
        ...
        let sphere_origin = FunctionOptimization::new(
            &vec![0.0.into(), 0.0.into()],
            &0.1.into(),
            TestFunction::Sphere,
        );
        ...
        let rosenbrock_min = FunctionOptimization::new(
            &vec![1.0.into(), 1.0.into()],
            &0.1.into(),
            TestFunction::Rosenbrock,
        );
        ...
        let booth_min = FunctionOptimization::new(
            &vec![1.0.into(), 3.0.into()],
            &0.1.into(),
            TestFunction::Booth,
        );
        assert!(
        ...
        let matyas_min = FunctionOptimization::new(
            &vec![0.0.into(), 0.0.into()],
            &0.1.into(),
            TestFunction::Matyas,
        );
    ...
    fn test_neighbor_generation() {
        let original = FunctionOptimization::new(
    ...
    fn test_tsp_properties() {
        let distances = vec![
            vec![MyFloat::new(0.0), 10.0.into(), 15.0.into(), 20.0.into()],
            vec![10.0.into(), 0.0.into(), 35.0.into(), 25.0.into()],
            vec![15.0.into(), 35.0.into(), 0.0.into(), 30.0.into()],
            vec![20.0.into(), 25.0.into(), 30.0.into(), 0.0.into()],
        ];

        let tsp = TSPSolution::new(&distances);
    ...
    fn test_acceptance_probability() {
        ...
        let prob_better = problem.acceptance_probability(&1.0.into(), &2.0.into(), &10.0.into());
        assert!(
        ...
        let prob_worse = problem.acceptance_probability(&2.0.into(), &1.0.into(), &10.0.into());
        assert!(
            prob_worse < 1.0,
        ...
        let prob_high_temp =
            problem.acceptance_probability(&2.0.into(), &1.0.into(), &100.0.into());
        let prob_low_temp = problem.acceptance_probability(&2.0.into(), &1.0.into(), &0.1.into());
        assert!(
    ...
    fn test_step_size_effects() {
        let step_sizes = vec![MyFloat::new(0.01), 0.1.into(), 1.0.into(), 10.0.into()];
        let trials = 5;
    ...
    fn test_multimodal_functions() {
        ...
        for func in multimodal_functions {
            ...
            let starting_points = vec![
                vec![MyFloat::new(0.0), 0.0.into()],
                vec![2.0.into(), 2.0.into()],
                vec![MyFloat::new(-2.0), MyFloat::new(-2.0)],
                vec![2.0.into(), MyFloat::new(-2.0)],
                vec![MyFloat::new(-2.0), 2.0.into()],
            ];
            ...
            for start in starting_points {
                ...
                let problem = FunctionOptimization::new(&start, &0.1.into(), func.clone());
                let result = optimizer.optimize(problem);
    ...
    fn test_improvement_over_time() {
        ...
        for &max_iters in &iteration_limits {
            let (_, avg_energy, _) = run_trials(
                ...
                5,
                &f64::INFINITY.into(), // We're just measuring average performance
            );
    ...
    fn test_extreme_values() {
        ...
        let large_initial = FunctionOptimization::new(
            &vec![1000.0.into(), MyFloat::new(-1000.0)],
            &1.0.into(),
            TestFunction::Sphere,
        );
        let mut optimizer = SimAnnealing::new(
            Some(10000.0.into()), // High temperature to handle large energy differences
            Some(0.1.into()),
            Some(0.99.into()),
            Some(100),
            Some(10000),
        ...
        let small_step = FunctionOptimization::new(
            &vec![1.0.into(), 1.0.into()],
            &0.001.into(),
            TestFunction::Sphere,
        );
        let mut fine_optimizer = SimAnnealing::new(
            Some(1.0.into()),
            Some(0.001.into()),
            Some(0.999.into()),
            Some(100),
    ...
    fn comprehensive_benchmark() {
        let test_cases = vec![
            ...
            (
                TestFunction::Sphere,
                vec![MyFloat::new(5.0), MyFloat::new(-3.0)],
                MyFloat::new(0.2),
                MyFloat::new(1.0),
                "Easy",
            ), // More lenient
        ...
        for (func, initial, step_size, threshold, difficulty) in test_cases {
            let (energies, avg_energy, successes) = run_trials(
                || FunctionOptimization::new(&initial, &step_size, func.clone()),
                || {
                    SimAnnealing::new(
                        Some(100.0.into()),
                        Some(0.001.into()),
    ...
    fn test_performance_characteristics() {
        ...
        for &size in &problem_sizes {
            let initial: Vec<MyFloat> = (0..size).map(|i| (i as f64).into()).collect();
            ...
            let problem = FunctionOptimization::new(&initial, &0.1.into(), TestFunction::Sphere);
            let result = optimizer.optimize(problem);
