...
pub trait Constraint: DynClone {
    ...
    fn gradient(&self, x: &Array1<MyFloat>) -> Array1<MyFloat>;
...
pub struct LinearConstraint {
    pub a: Array1<MyFloat>,
    pub b: MyFloat,
    pub is_equality: bool,
}

impl LinearConstraint {
    pub fn new(a: &Array1<MyFloat>, b: &MyFloat, is_equality: bool) -> Self {
        Self {
            a: a.clone(),
            b: b.clone(),
            is_equality,
    ...
    pub fn inequality(a: &Array1<MyFloat>, b: &MyFloat) -> Self {
        Self::new(a, b, false)
    }

    pub fn equality(a: &Array1<MyFloat>, b: &MyFloat) -> Self {
        Self::new(a, b, true)
    }
}
...
impl QuadraticConstraint {
    pub fn new(q: &Array2<MyFloat>, a: &Array1<MyFloat>, b: &MyFloat, is_equality: bool) -> Self {
        Self {
            q: q.clone(),
            a: a.clone(),
            b: b.clone(),
...
impl Constraint for QuadraticConstraint {
    ...
    fn hessian(&self, _x: &Array1<MyFloat>) -> Array2<MyFloat> {
...
pub fn create_box_constraints(
    lower: &Array1<MyFloat>,
    upper: &Array1<MyFloat>,
    ...
    for (i, (l, u)) in lower.iter().zip(upper.iter()).enumerate() {
        ...
        if l.is_finite() {
            let mut a = Array1::<MyFloat>::zeros(lower.len());
            a[i] = MyFloat::new(-1.0);
        ...
        if u.is_finite() {
            ...
            a[i] = MyFloat::new(1.0);
            constraints
                .push(Box::new(LinearConstraint::inequality(&a, &(-1_f64 * u)))
...
#[cfg(test)]
mod minimize_myfloat_constraint_tests {
    use super::*;
    ...
    fn test_linear_constraint() {
        let constraint = LinearConstraint::inequality(
            &array![MyFloat::one(), MyFloat::one()],
            &MyFloat::neg_one(),
        );
        let x = array![0.5.into(), 0.3.into()];

        // Test constraint: x1 + x2 - 1 ≤ 0
        approx_eq!(f64, constraint.evaluate(&x).to_f64(), -0.2); // 0.5 + 0.3 - 1 = -0.2
        assert_eq!(constraint.gradient(&x), array![1.0, 1.0]);

        let hess = constraint.hessian(&x);
        assert_eq!(hess, Array2::<MyFloat>::zeros((2, 2)));
    }
    ...
    fn test_quadratic_constraint() {
        ...
        let constraint = QuadraticConstraint::new(&q, &a, &MyFloat::neg_one(), false);
        let x = array![0.5.into(), 0.5.into()];

        // Test constraint: x1² + x2² - 1 ≤ 0
        assert!((constraint.evaluate(&x).to_f64()).abs() < 1e-10); // 1.0 + 0.0 - 1 = 0.0
        assert_eq!(constraint.gradient(&x), array![2.0, 2.0]); // [4*0.5, 4*0.5]

        let hess = constraint.hessian(&x);
        assert_eq!(hess, Array2::<MyFloat>::eye(2) * 4.0);
    }

    #[test]
    fn test_box_constraints() {
        let lower = array![0.0.into(), MyFloat::neg_one()];
        let upper = array![2.0.into(), MyFloat::one()];
        let constraints = create_box_constraints(&lower, &upper);

        assert_eq!(constraints.len(), 4); // 2 lower + 2 upper bounds

        let x = array![MyFloat::one(), 0.0.into()];
        ...
        // Test boundary point
        let x_boundary = array![0.0.into(), MyFloat::one()];
        for constraint in &constraints {
            assert!(
    ...
    fn create_identity_matrix(n: usize) -> Array2<MyFloat> {
    ...
    fn create_symmetric_matrix(values: &[(usize, usize, MyFloat)]) -> Array2<MyFloat> {
        let max_idx = values
            .iter()
            .map(|(i, j, _)| (*i).max(*j))
            .max()
        ...
        for (i, j, val) in values {
            ...
            if *i != *j {
                matrix[[*j, *i]] = val.clone(); // Ensure symmetry
            }
        }
        matrix
    }
    ...
    mod linear_constraint_tests {
        ...
        fn test_linear_inequality_basic() {
            ...
            // Test constraint: x1 + x2 - 1 ≤ 0
            approx_eq!(f64, constraint.evaluate(&x).to_f64(), -0.2);
            assert_eq!(
        ...
        fn test_linear_equality_basic() {
            let constraint =
                LinearConstraint::equality(&array![2.0.into(), MyFloat::neg_one()], &3.0.into());
            let x = array![2.0.into(), MyFloat::one()];

            // Test constraint: 2*x1 - x2 + 3 = 0
            approx_eq!(f64, constraint.evaluate(&x).to_f64(), 6.0); // 2*2 - 1 + 3 = 6
            assert_eq!(
                constraint.gradient(&x),
                array![2.0.into(), MyFloat::neg_one()]
            );
            assert!(constraint.is_equality);
        }
        ...
        fn test_linear_constraint_zero_coefficients() {
            let constraint = LinearConstraint::inequality(
                &array![0.0.into(), 0.0.into(), MyFloat::one()],
                &(-5.0 * MyFloat::one()),
            );
            let x = array![10.0.into(), 20.0.into(), 3.0.into()];

            // Test constraint: 0*x1 + 0*x2 + 1*x3 - 5 ≤ 0
            approx_eq!(f64, constraint.evaluate(&x).to_f64(), -2.0); // 0 + 0 + 3 - 5 = -2
            assert_eq!(
                constraint.gradient(&x),
                array![0.0.into(), 0.0.into(), MyFloat::one()]
            );
        ...
        fn test_linear_constraint_negative_coefficients() {
            let constraint = LinearConstraint::inequality(
                &array![-2.0 * MyFloat::one(), -3.0 * MyFloat::one()],
                &10.0.into(),
            );
            let x = array![MyFloat::one(), 2.0.into()];

            // Test constraint: -2*x1 - 3*x2 + 10 ≤ 0
            approx_eq!(f64, constraint.evaluate(&x).to_f64(), 2.0); // -2 - 6 + 10 = 2
        }

        #[test]
        fn test_linear_constraint_single_variable() {
            let constraint =
                LinearConstraint::inequality(&array![5.0.into()], &(-10.0 * MyFloat::one()));
            let x = array![3.0.into()];

            approx_eq!(f64, constraint.evaluate(&x).to_f64(), 5.0); // 5*3 - 10 = 5
        ...
        fn test_linear_constraint_large_dimension() {
            let n = 100;
            let a: Array1<MyFloat> = (0..n).map(|i| MyFloat::new(i as f64)).collect();
            let x: Array1<MyFloat> = Array1::ones(n);
            let constraint = LinearConstraint::inequality(&a, &(-50.0 * MyFloat::one()));

            let expected = (0..n).sum::<usize>() as f64 - 50.0; // Sum of 0..99 = 4950
            approx_eq!(f64, constraint.evaluate(&x).to_f64(), expected);
        ...
        fn test_linear_constraint_at_boundary() {
            ...
            let x = array![MyFloat::one(), MyFloat::one()]; // This should make constraint exactly 0

            approx_eq!(f64, constraint.evaluate(&x).to_f64(), 0.0);
        }

        #[test]
        fn test_linear_constraint_debug_format() {
            let ineq = LinearConstraint::inequality(
                &array![MyFloat::one(), -2.0 * MyFloat::one()],
                &3.0.into(),
            );
            let eq = LinearConstraint::equality(
                &array![0.5.into(), 1.5.into()],
                &(-1.0 * MyFloat::one()),
            );
            ...
            assert!(
                ineq_debug.contains("[MyFloat(1.0000000000000000), MyFloat(-2.0000000000000000)]")
            );
            assert!(
                eq_debug.contains("[MyFloat(5.0000000000000000e-1), MyFloat(1.5000000000000000)]")
            );
        }
    }
    ...
    mod quadratic_constraint_tests {
        ...
        fn test_quadratic_constraint_identity_matrix() {
            ...
            let a = array![0.0.into(), 0.0.into()];
            let constraint = QuadraticConstraint::new(&q, &a, &(-1.0 * MyFloat::one()), false);
            let x = array![0.5.into(), 0.5.into()];

            // Test constraint: x1² + x2² - 1 ≤ 0
            approx_eq!(f64, constraint.evaluate(&x).to_f64(), -0.5); // 0.25 + 0.25 - 1 = -0.5
            assert_eq!(
                constraint.gradient(&x),
                array![MyFloat::one(), MyFloat::one()]
            ); // [2*0.5, 2*0.5]
        ...
        fn test_quadratic_constraint_with_linear_term() {
            let q = array![[MyFloat::one(), 0.0.into()], [0.0.into(), MyFloat::one()]];
            let a = array![2.0.into(), -1.0 * MyFloat::one()];
            let constraint = QuadraticConstraint::new(&q, &a, &3.0.into(), true);
            let x = array![MyFloat::one(), 2.0.into()];
        ...
        fn test_quadratic_constraint_cross_terms() {
            let q = array![[MyFloat::one(), 0.5.into()], [0.5.into(), 2.0.into()]];
            let a = array![0.0.into(), 0.0.into()];
            let constraint = QuadraticConstraint::new(&q, &a, &0.0.into(), false);
            let x = array![MyFloat::one(), MyFloat::one()];
        ...
        fn test_quadratic_constraint_zero_quadratic_term() {
            let q = array![[0.0.into(), 0.0.into()], [0.0.into(), 0.0.into()]];
            let a = array![3.0.into(), -2.0 * MyFloat::one()];
            let constraint = QuadraticConstraint::new(&q, &a, &MyFloat::one(), false);
            let x = array![2.0.into(), MyFloat::one()];

            // This should behave like a linear constraint: 3*x1 - 2*x2 + 1 ≤ 0
            approx_eq!(f64, constraint.evaluate(&x).to_f64(), 5.0); // 6 - 2 + 1 = 5
            assert_eq!(
                constraint.gradient(&x),
                array![3.0.into(), -2.0 * MyFloat::one()]
            );
        ...
        fn test_quadratic_constraint_single_variable() {
            let q = array![[2.0.into()]];
            let a = array![-4.0 * MyFloat::one()];
            let constraint = QuadraticConstraint::new(&q, &a, &MyFloat::one(), false);
            let x = array![3.0.into()];
        ...
        fn test_quadratic_constraint_large_dimension() {
            ...
            let constraint = QuadraticConstraint::new(&q, &a, &(-5.0 * MyFloat::one()), false);
            let x = Array1::ones(n);

            // Test constraint: sum(xi²) - 5 ≤ 0
            approx_eq!(f64, constraint.evaluate(&x).to_f64(), 5.0); // 10 - 5 = 5

            let grad = constraint.gradient(&x);
            assert_eq!(grad, Array1::<MyFloat>::ones(n) * 2.0); // 2*xi for each i
        }

        #[test]
        fn test_quadratic_constraint_debug_format() {
            let q = array![[MyFloat::one(), 0.0.into()], [0.0.into(), 2.0.into()]];
            let a = array![MyFloat::one(), -1.0 * MyFloat::one()];
            let ineq = QuadraticConstraint::new(&q, &a, &0.5.into(), false);
            let eq = QuadraticConstraint::new(&q, &a, &0.5.into(), true);
            ...
            assert!(ineq_debug.contains("≤"));
            assert!(eq_debug.contains("="));
            assert!(ineq_debug.contains("MyFloat(5.0000000000000000e-1)"));
        }
    }
    ...
    mod box_constraint_tests {
        ...
        fn test_box_constraints_basic() {
            let lower = array![0.0.into(), -1.0 * MyFloat::one()];
            let upper = array![2.0.into(), MyFloat::one()];
            let constraints = create_box_constraints(&lower, &upper);
        ...
        fn test_box_constraints_at_boundaries() {
            let lower = array![-2.0 * MyFloat::one(), 0.0.into()];
            let upper = array![3.0.into(), 5.0.into()];
            let constraints = create_box_constraints(&lower, &upper);

            // Test lower boundary
            let x_lower = array![-2.0 * MyFloat::one(), 0.0.into()];
            for constraint in &constraints {
                assert!(
            ...
            // Test upper boundary
            let x_upper = array![3.0.into(), 5.0.into()];
            for constraint in &constraints {
                assert!(
        ...
        fn test_box_constraints_violation() {
            let lower = array![MyFloat::one(), 2.0.into()];
            let upper = array![4.0.into(), 6.0.into()];
            let constraints = create_box_constraints(&lower, &upper);

            // Test point below lower bound
            let x_below = array![0.5.into(), 3.0.into()];
            let violations_below: Array1<MyFloat> = constraints
                .iter()
            ...
            let x_above = array![2.0.into(), 7.0.into()];
            let violations_above: Array1<MyFloat> = constraints
                .iter()
        ...
        fn test_box_constraints_infinite_bounds() {
            let lower = array![MyFloat::new(f64::NEG_INFINITY), 0.0.into()];
            let upper = array![5.0.into(), MyFloat::new(f64::INFINITY)];
            let constraints = create_box_constraints(&lower, &upper);

            // Should only create constraints for finite bounds
            assert_eq!(constraints.len(), 2); // One lower bound, one upper bound

            let x = array![-1000.0 * MyFloat::one(), 1000.0.into()]; // Large values
            for constraint in &constraints {
        ...
        fn test_box_constraints_single_variable() {
            let lower = array![2.0.into()];
            let upper = array![8.0.into()];
            let constraints = create_box_constraints(&lower, &upper);

            assert_eq!(constraints.len(), 2);

            let x_valid = array![5.0.into()];
            for constraint in &constraints {
                assert!(constraint.evaluate(&x_valid) <= 1e-10);
            }

            let x_invalid_low = array![MyFloat::one()];
            let violations_low: Array1<MyFloat> = constraints
                .iter()
            ...
            let x_invalid_high = array![10.0.into()];
            let violations_high: Array1<MyFloat> = constraints
                .iter()
        ...
        fn test_box_constraints_empty_bounds() {
            let lower: Array1<MyFloat> = array![];
            let upper: Array1<MyFloat> = array![];
            let constraints = create_box_constraints(&lower, &upper);
        ...
        fn test_box_constraints_equal_bounds() {
            let lower = array![5.0.into(), -2.0 * MyFloat::one()];
            let upper = array![5.0.into(), -2.0 * MyFloat::one()];
            let constraints = create_box_constraints(&lower, &upper);

            // Should create both lower and upper bounds even when equal
            assert_eq!(constraints.len(), 4);

            let x_exact = array![5.0.into(), -2.0 * MyFloat::one()];
            for constraint in &constraints {
                assert!(
        ...
        fn test_box_constraints_gradients() {
            let lower = array![0.0.into(), -1.0 * MyFloat::one()];
            let upper = array![2.0.into(), 3.0.into()];
            let constraints = create_box_constraints(&lower, &upper);
            let x = array![MyFloat::one(), MyFloat::one()];
    ...
    mod integration_tests {
        ...
        fn test_constraint_cloning() {
            let linear = LinearConstraint::inequality(
                &array![MyFloat::one(), 2.0.into()],
                &(-1.0 * MyFloat::one()),
            );
            let cloned_linear = linear.clone();

            let x = array![0.5.into(), 0.25.into()];
            assert_eq!(linear.evaluate(&x), cloned_linear.evaluate(&x));
            assert_eq!(linear.gradient(&x), cloned_linear.gradient(&x));

            let q = array![[MyFloat::one(), 0.5.into()], [0.5.into(), 2.0.into()]];
            let quadratic = QuadraticConstraint::new(
                &q,
                &array![MyFloat::one(), -1.0 * MyFloat::one()],
                &2.0.into(),
                false,
            );
        ...
        fn test_mixed_constraint_system() {
            ...
            let mut box_constraints = create_box_constraints(
                &array![0.0.into(), 0.0.into()],
                &array![MyFloat::one(), MyFloat::one()],
            );
            ...
            // Test feasible point
            let x_feasible = array![0.5.into(), 0.5.into()];
            for constraint in &constraints {
                assert!(
            ...
            let x_infeasible = array![1.5.into(), 1.5.into()];
            let violations: Array1<MyFloat> = constraints
                .iter()
        ...
        fn test_constraint_consistency() {
            ...
            let constraint = QuadraticConstraint::new(
                &array![[2.0.into(), MyFloat::one()], [MyFloat::one(), 3.0.into()]],
                &array![MyFloat::one(), -2.0 * MyFloat::one()],
                &0.5.into(),
                false,
            );

            let x = array![1.0.into(), 0.5.into()];
            let h = &1e-8.into();
            ...
            for i in 0..x.len() {
                ...
                approx_eq!(f64, grad[i].to_f64(), finite_diff.to_f64(), epsilon = 1e-5);
            }
        }
        ...
        fn test_zero_dimensional_constraint() {
            let constraint = LinearConstraint::inequality(&array![], &5.0.into());
            let x: Array1<MyFloat> = array![];

            // Should evaluate to just the constant term
            approx_eq!(f64, constraint.evaluate(&x).to_f64(), 5.0);
            assert_eq!(constraint.gradient(&x), Array1::<MyFloat>::zeros(0));
            assert_eq!(constraint.hessian(&x), Array2::<MyFloat>::zeros((0, 0)));
        }

        #[test]
        fn test_constraint_with_extreme_values() {
            let constraint = LinearConstraint::inequality(
                &array![1e10.into(), -1e10 * MyFloat::one()],
                &1e15.into(),
            );
            let x = array![1e-10.into(), 1e-10.into()];
    ...
    mod performance_tests {
        ...
        fn test_large_linear_constraint_performance() {
            let n = 10000;
            let a: Array1<MyFloat> = (0..n).map(|i| MyFloat::new(i as f64).sin()).collect();
            let x: Array1<MyFloat> = (0..n).map(|i| MyFloat::new(i as f64).cos()).collect();
            let constraint = LinearConstraint::inequality(&a, &(-100.0 * MyFloat::one()));
