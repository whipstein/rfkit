...
#[derive(Debug, Clone)]
pub struct SimplexResult<T: RealScalar> {
    xmin: Points1<T>,
    fmin: T,
    iters: usize,
    fn_evals: usize,
...
impl<T: RealScalar> SimplexResult<T> {
...
pub struct Simplex<T: RealScalar> {
    xmin: Points1<T>,
    fmin: T,
    f: Box<dyn ObjFn<T>>,
    iters: usize,
...
impl<T: RealScalar> Clone for Simplex<T> {
    fn clone(&self) -> Self {
        Self {
...
impl<T: RealScalar> Simplex<T> {
    ...
    pub fn minimize_with_steps(
        ...
        self.run_simplex_algorithm(simplex, &scl, 1e-8.into(), 1000)
    }
}

impl<T: RealScalar> fmt::Debug for Simplex<T> {
...
mod minimize_simplex_tests {
    ...
    fn ackley(bias: f64) -> F1dim<TwoFloat> {
    ...
    fn elliptic(bias: f64) -> F1dim<TwoFloat> {
        F1dim::new(MultiDimFn::new(move |x: &Points1<TwoFloat>| {
            let sum: TwoFloat = x
                .iter()
                .enumerate()
    ...
    fn griewank(bias: f64) -> F1dim<TwoFloat> {
        F1dim::new(MultiDimFn::new(move |x: &Points1<TwoFloat>| {
    ...
    fn rosenbrock(shift: Points1<TwoFloat>, bias: f64) -> F1dim<TwoFloat> {
        F1dim::new(MultiDimFn::new(move |x: &Points1<TwoFloat>| {
    ...
    fn test_rosenbrock_2d() {
        ...
        let rosenbrock =
            |x: &Points1<TwoFloat>| (1.0 - &x[0]).powi(2) + 100.0 * (&x[1] - &x[0].powi(2)).powi(2);
        let objective = MultiDimFn::new(rosenbrock);
        let mut simplex = Simplex::new(objective);

        let result = simplex
            .downhill_simplex(
                &array![TwoFloat::from(-1.2), 1.0.into()].into(),
                None,
                Some(0.1.into()),
                Some(1e-6.into()),
                Some(2000),
    ...
    fn test_custom_step_sizes() {
        let func = |x: &Points1<TwoFloat>| (&x[0] / 10.0 - 1.0).powi(2) + (&x[1] - 2.0).powi(2);
        let objective = MultiDimFn::new(func);
        let mut simplex = Simplex::new(objective);
    ...
    fn test_simplex_high_dimensional() {
        ...
        let result = simplex
            .minimize(&Points1::ones(5), None, None, None, None)
            .unwrap();
    ...
    fn test_simplex_rosenbrock_variants() {
        ...
        let rosenbrock_4d = |x: &Points1<TwoFloat>| {
            (0..x.len() - 1)
    ...
    fn test_simplex_with_constraints_penalty() {
        ...
        let constrained_objective = |x: &Points1<TwoFloat>| {
    ...
    mod basic_functionality_tests {
        ...
        fn test_negative_coordinates() {
            // Minimum at (-3, -4)
            let func = |x: &Points1<TwoFloat>| (&x[0] + 3.0).powi(2) + (&x[1] + 4.0).powi(2);
            let objective = MultiDimFn::new(func);
            let mut simplex = Simplex::new(objective);
    ...
    mod error_handling_tests {
        ...
        fn test_function_returning_nan() {
            ...
            let result =
                simplex.minimize(&array![TwoFloat::from(-1.0)].into(), None, None, None, None); // Start where function is NaN
            assert!(result.is_err());
            assert!(matches!(
                result.unwrap_err(),
                MinimizerError::FunctionEvaluationError
    ...
    mod high_dimensional_tests {
        ...
        fn test_10d_sphere() {
            ...
            let result = simplex
                .minimize(&Points1::ones(10), None, None, None, None)
                .unwrap();
    ...
    mod complex_function_tests {
        ...
        fn test_goldstein_price_function() {
            ...
            let goldstein_price = |x: &Points1<TwoFloat>| {
                let x1 = x[0].clone();
                let x2 = x[1].clone();
            ...
            let result = simplex
                .downhill_simplex(
                    &array![0.5.into(), TwoFloat::from(-0.5)].into(),
                    None,
                    Some(0.2.into()),
                    Some(1e-6.into()),
                    Some(3000),
        ...
        fn test_himmelblau_function() {
            ...
            let himmelblau = |x: &Points1<TwoFloat>| {
            ...
            let starting_points = array![
                ...
                array![TwoFloat::from(-3.0), 3.0.into()],
                array![TwoFloat::from(-3.0), TwoFloat::from(-3.0)],
            ];
            ...
            for start in starting_points {
                ...
                let known_minima = array![
                    (3.0.into(), 2.0.into()),
                    (TwoFloat::from(-2.805118), 3.131312.into()),
                    (TwoFloat::from(-3.779310), TwoFloat::from(-3.283186)),
                    (3.584428.into(), TwoFloat::from(-1.848126)),
                ];
    ...
    mod adaptive_vs_standard_comparison_tests {
        ...
        fn test_adaptive_no_improvement_convergence() {
            ...
            let plateau_func = |x: &Points1<TwoFloat>| {
    ...
    mod custom_step_size_tests {
        ...
        fn test_custom_step_sizes_scaled_problem() {
            ...
            let scaled_func = |x: &Points1<TwoFloat>| {
    ...
    mod edge_case_function_tests {
        ...
        fn test_constant_function() {
            let constant_func = |_: &Points1<TwoFloat>| 42.0.into();
        ...
        fn test_linear_function() {
            ...
            let linear_func = |x: &Points1<TwoFloat>| 2.0 * &x[0] + 3.0 * &x[1];
        ...
        fn test_discontinuous_function() {
            let discontinuous = |x: &Points1<TwoFloat>| {
                if x[0] < 0.0 {
    ...
    mod numerical_stability_tests {
        ...
        fn test_ill_conditioned_problem() {
            ...
            let ill_conditioned = |x: &Points1<TwoFloat>| x[0].powi(2) + 10000.0 * x[1].powi(2);
        ...
        fn test_near_zero_function_values() {
            let tiny_func = |x: &Points1<TwoFloat>| 1e-15 * (x[0].powi(2) + x[1].powi(2));
    ...
    mod cec2005_tests {
        ...
        fn test_f1() {
            ...
            // 10d
            let n = 10;
            let x = Points(Array1::from_shape_fn(n, |_| {
                TwoFloat::from(rand::rng().random::<f64>() * 200.0 - 100.0)
            }));
        ...
        fn test_f2() {
            ...
            let func = move |x: &Points1<TwoFloat>| {
                x.iter()
                    .enumerate()
                    .map(|(i, _)| {
                        let mut new_val = TwoFloat::from(0.0);
                        for j in 0..=i {
        ...
        #[ignore]
        fn test_f3() {
            let shift: Points1<TwoFloat> = array![
                TwoFloat::from(-3.2201300e+001),
                6.4977600e+001.into(),
                (-3.8300000e+001).into(),
                (-2.3258200e+001).into(),
                (-5.4008800e+001).into(),
            ...
            let m = array![
                [
                    TwoFloat::from(1.7830682721057345e-001),
                    TwoFloat::from(5.5786330587166588e-002),
                    TwoFloat::from(4.7591905576669730e-001),
                    TwoFloat::from(2.4551129863391566e-001),
                    TwoFloat::from(3.1998625926387086e-001),
                    TwoFloat::from(3.2102001448363848e-001),
                    TwoFloat::from(2.7787561319902176e-002),
                    TwoFloat::from(2.6664001046775621e-001),
                    TwoFloat::from(4.1568009651337917e-001),
                    TwoFloat::from(-4.7771934552669726e-001)
                ],
                [
                    TwoFloat::from(6.3516362859468667e-001),
                    TwoFloat::from(5.0091423836646241e-002),
                    TwoFloat::from(2.0110601384121973e-001),
                    TwoFloat::from(-6.8076882416633511e-001),
                    TwoFloat::from(-4.9934546553907944e-002),
                    TwoFloat::from(-4.6399423424582961e-002),
                    TwoFloat::from(-1.9460194646748039e-001),
                    TwoFloat::from(1.8961539926194687e-001),
                    TwoFloat::from(-1.9416259626804547e-002),
                    TwoFloat::from(1.0639981029473855e-001)
                ],
                [
                    TwoFloat::from(3.2762147366023187e-001),
                    TwoFloat::from(3.6016598714114556e-001),
                    TwoFloat::from(-2.3635655094044949e-001),
                    TwoFloat::from(-1.8566854017444848e-002),
                    TwoFloat::from(-2.4479096747593634e-001),
                    TwoFloat::from(4.4818973341886903e-001),
                    TwoFloat::from(5.3518635733619568e-001),
                    TwoFloat::from(-3.1206925190530521e-001),
                    TwoFloat::from(-1.3863719921728737e-001),
                    TwoFloat::from(-2.0713981146209595e-001)
                ],
                [
                    TwoFloat::from(-6.4783210587984280e-002),
                    TwoFloat::from(-4.9424101683695937e-001),
                    TwoFloat::from(1.3101175297435969e-001),
                    TwoFloat::from(3.1615171931194543e-002),
                    TwoFloat::from(-1.7506107914871860e-001),
                    TwoFloat::from(6.8908039344918381e-001),
                    TwoFloat::from(1.0544234469094992e-002),
                    TwoFloat::from(2.1948984793273507e-001),
                    TwoFloat::from(-1.6468539805844565e-001),
                    TwoFloat::from(3.9048550518513409e-001)
                ],
                [
                    TwoFloat::from(-2.7648044785371367e-001),
                    TwoFloat::from(1.1383114506120220e-001),
                    TwoFloat::from(-3.0818401502810994e-001),
                    TwoFloat::from(-3.5959407104438740e-001),
                    TwoFloat::from(2.6446258034702191e-001),
                    TwoFloat::from(2.8616788379157501e-002),
                    TwoFloat::from(4.7528027904995646e-001),
                    TwoFloat::from(4.0993994049770172e-001),
                    TwoFloat::from(4.1131043368915432e-001),
                    TwoFloat::from(2.2899345188886880e-001)
                ],
                [
                    TwoFloat::from(1.5454249061641606e-001),
                    TwoFloat::from(5.4899186274157996e-001),
                    TwoFloat::from(-1.8382029941792261e-001),
                    TwoFloat::from(3.3944461903909162e-001),
                    TwoFloat::from(2.8596188774255699e-001),
                    TwoFloat::from(1.2833167642713417e-001),
                    TwoFloat::from(-2.5495080172376317e-001),
                    TwoFloat::from(3.9460752302037100e-001),
                    TwoFloat::from(-3.4524640270007412e-001),
                    TwoFloat::from(2.9590318323368509e-001)
                ],
                [
                    TwoFloat::from(-5.1907977690014512e-002),
                    TwoFloat::from(-1.4450757809700329e-001),
                    TwoFloat::from(-4.6086919626114314e-001),
                    TwoFloat::from(-5.3687964818368079e-002),
                    TwoFloat::from(-3.6317793499109247e-001),
                    TwoFloat::from(2.7439997038558633e-002),
                    TwoFloat::from(-2.1422629652542946e-001),
                    TwoFloat::from(5.0545148893084779e-001),
                    TwoFloat::from(-9.8064717019089837e-002),
                    TwoFloat::from(-5.6346991018564507e-001)
                ],
                [
                    TwoFloat::from(5.0142989354460654e-001),
                    TwoFloat::from(-5.3133659048457516e-001),
                    TwoFloat::from(-3.7294385871521135e-001),
                    TwoFloat::from(2.3370866431381510e-001),
                    TwoFloat::from(4.4327537662488531e-001),
                    TwoFloat::from(-1.6972740381143742e-001),
                    TwoFloat::from(2.0364148963331691e-001),
                    TwoFloat::from(-2.3717523924336927e-002),
                    TwoFloat::from(-7.1805455862954920e-002),
                    TwoFloat::from(-7.3332178450339763e-003)
                ],
                [
                    TwoFloat::from(1.0441248047680891e-001),
                    TwoFloat::from(4.3064226149369542e-002),
                    TwoFloat::from(-4.1675972625940993e-001),
                    TwoFloat::from(1.6522876074361707e-002),
                    TwoFloat::from(1.7437281849141879e-003),
                    TwoFloat::from(2.9594944879030760e-001),
                    TwoFloat::from(-5.1197487739368741e-001),
                    TwoFloat::from(-3.2679819762357892e-001),
                    TwoFloat::from(5.8253106590933512e-001),
                    TwoFloat::from(1.3204141339826148e-001)
                ],
                [
                    TwoFloat::from(-2.9645907657631693e-001),
                    TwoFloat::from(-3.1303011496605505e-002),
                    TwoFloat::from(-7.8009154082116602e-002),
                    TwoFloat::from(-4.1548534874482024e-001),
                    TwoFloat::from(5.6959403572443468e-001),
                    TwoFloat::from(2.9095198400348149e-001),
                    TwoFloat::from(-1.8560717510075503e-001),
                    TwoFloat::from(-2.4653488847859115e-001),
                    TwoFloat::from(-3.7149025085479792e-001),
                    TwoFloat::from(-3.0015617693118707e-001)
                ],
            ];
            let bias = -450.0;
        ...
        fn test_f4() {
            ...
            let shift = array![
                TwoFloat::from(3.5626700e+001),
                TwoFloat::from(-8.2912300e+001),
                TwoFloat::from(-1.0642300e+001),
                TwoFloat::from(-8.3581500e+001),
                TwoFloat::from(8.3155200e+001),
                TwoFloat::from(4.7048000e+001),
                TwoFloat::from(-8.9435900e+001),
                TwoFloat::from(-2.7421900e+001),
                TwoFloat::from(7.6144800e+001),
                TwoFloat::from(-3.9059500e+001),
                TwoFloat::from(4.8885700e+001),
                TwoFloat::from(-3.9828000e+000),
                TwoFloat::from(-7.1924300e+001),
                TwoFloat::from(6.4194700e+001),
                TwoFloat::from(-4.7733800e+001),
                TwoFloat::from(-5.9896000e+000),
                TwoFloat::from(-2.6282800e+001),
                TwoFloat::from(-5.9181100e+001),
                TwoFloat::from(1.4602800e+001),
                TwoFloat::from(-8.5478000e+001),
                TwoFloat::from(-5.0490100e+001),
                TwoFloat::from(9.2400000e-001),
                TwoFloat::from(3.2397800e+001),
                TwoFloat::from(3.0238800e+001),
                TwoFloat::from(-8.5094900e+001),
                TwoFloat::from(6.0119700e+001),
                TwoFloat::from(-3.6218300e+001),
                TwoFloat::from(-8.5883000e+000),
                TwoFloat::from(-5.1971000e+000),
                TwoFloat::from(8.1553100e+001),
                TwoFloat::from(-2.3431600e+001),
                TwoFloat::from(-2.5350500e+001),
                TwoFloat::from(-4.1248500e+001),
                TwoFloat::from(8.8018000e+000),
                TwoFloat::from(-2.4222200e+001),
                TwoFloat::from(-8.7980700e+001),
                TwoFloat::from(7.8047300e+001),
                TwoFloat::from(-4.8052800e+001),
                TwoFloat::from(1.4017700e+001),
                TwoFloat::from(-3.6640500e+001),
                TwoFloat::from(1.2216800e+001),
                TwoFloat::from(1.8144900e+001),
                TwoFloat::from(-6.4564700e+001),
                TwoFloat::from(-8.4849300e+001),
                TwoFloat::from(-7.6608800e+001),
                TwoFloat::from(-1.7042000e+000),
                TwoFloat::from(-3.6076100e+001),
                TwoFloat::from(3.7033600e+001),
                TwoFloat::from(1.8443100e+001),
                TwoFloat::from(-6.4359000e+001),
                TwoFloat::from(-3.9369200e+001),
                TwoFloat::from(-1.7714000e+001),
                TwoFloat::from(3.0198500e+001),
                TwoFloat::from(-1.8548300e+001),
                TwoFloat::from(9.6866000e+000),
                TwoFloat::from(8.2600900e+001),
                TwoFloat::from(-4.5525600e+001),
                TwoFloat::from(5.1443000e+000),
                TwoFloat::from(7.4204000e+001),
                TwoFloat::from(6.6810300e+001),
                TwoFloat::from(-6.3470400e+001),
                TwoFloat::from(1.3032900e+001),
                TwoFloat::from(-5.6878000e+000),
                TwoFloat::from(2.9527100e+001),
                TwoFloat::from(-4.3530000e-001),
                TwoFloat::from(-2.6165200e+001),
                TwoFloat::from(-6.6847000e+000),
                TwoFloat::from(-8.0229100e+001),
                TwoFloat::from(-2.9581500e+001),
                TwoFloat::from(8.2042200e+001),
                TwoFloat::from(7.7177000e+001),
                TwoFloat::from(-1.1277000e+001),
                TwoFloat::from(3.2075900e+001),
                TwoFloat::from(-2.6858000e+000),
                TwoFloat::from(8.1509600e+001),
                TwoFloat::from(6.4077000e+001),
                TwoFloat::from(-2.6129400e+001),
                TwoFloat::from(-8.4782000e+001),
                TwoFloat::from(-6.2876800e+001),
                TwoFloat::from(-3.7635500e+001),
                TwoFloat::from(7.6891600e+001),
                TwoFloat::from(5.3417000e+001),
                TwoFloat::from(-2.5331100e+001),
                TwoFloat::from(-3.8070200e+001),
                TwoFloat::from(-8.4173800e+001),
                TwoFloat::from(-1.1224600e+001),
                TwoFloat::from(-8.3461900e+001),
                TwoFloat::from(-1.7550800e+001),
                TwoFloat::from(-3.6528500e+001),
                TwoFloat::from(8.9552800e+001),
                TwoFloat::from(2.5879400e+001),
                TwoFloat::from(6.8625200e+001),
                TwoFloat::from(5.5796800e+001),
                TwoFloat::from(-2.9597500e+001),
                TwoFloat::from(-5.8097600e+001),
                TwoFloat::from(6.5741300e+001),
                TwoFloat::from(-8.8703000e+000),
                TwoFloat::from(-5.3281000e+000),
                TwoFloat::from(7.4066100e+001),
                TwoFloat::from(4.0338000e+000)
            ];
            let bias = -450.0;
            ...
            let shift_func = shift.clone();
            let rand_val = TwoFloat::from(rng.random::<f64>());
            let func = move |x: &Points1<TwoFloat>| {
                x.iter()
                    .enumerate()
                    .map(|(i, _)| {
        ...
        fn test_f6() {
            let shift = array![
                TwoFloat::from(8.1023200e+001),
                TwoFloat::from(-4.8395000e+001),
                TwoFloat::from(1.9231600e+001),
                TwoFloat::from(-2.5231000e+000),
                TwoFloat::from(7.0433800e+001),
                TwoFloat::from(4.7177400e+001),
                TwoFloat::from(-7.8358000e+000),
                TwoFloat::from(-8.6669300e+001),
                TwoFloat::from(5.7853200e+001),
                TwoFloat::from(-9.9533000e+000),
                TwoFloat::from(2.0777800e+001),
                TwoFloat::from(5.2548600e+001),
                TwoFloat::from(7.5926300e+001),
                TwoFloat::from(4.2877300e+001),
                TwoFloat::from(-5.8272000e+001),
                TwoFloat::from(-1.6972800e+001),
                TwoFloat::from(7.8384500e+001),
                TwoFloat::from(7.5042700e+001),
                TwoFloat::from(-1.6151300e+001),
                TwoFloat::from(7.0856900e+001),
                TwoFloat::from(-7.9579500e+001),
                TwoFloat::from(-2.6483700e+001),
                TwoFloat::from(5.6369900e+001),
                TwoFloat::from(-8.8224900e+001),
                TwoFloat::from(-6.4999600e+001),
                TwoFloat::from(-5.3502200e+001),
                TwoFloat::from(-5.4230000e+001),
                TwoFloat::from(1.8682600e+001),
                TwoFloat::from(-4.1006100e+001),
                TwoFloat::from(-5.4213400e+001),
                TwoFloat::from(-8.7250600e+001),
                TwoFloat::from(4.4421400e+001),
                TwoFloat::from(-9.8826000e+000),
                TwoFloat::from(7.7726600e+001),
                TwoFloat::from(-6.1210000e+000),
                TwoFloat::from(-1.4643000e+001),
                TwoFloat::from(6.2319800e+001),
                TwoFloat::from(4.5274000e+000),
                TwoFloat::from(-5.3523400e+001),
                TwoFloat::from(3.0984700e+001),
                TwoFloat::from(6.0861300e+001),
                TwoFloat::from(-8.6464800e+001),
                TwoFloat::from(3.2629800e+001),
                TwoFloat::from(-2.1693400e+001),
                TwoFloat::from(5.9723200e+001),
                TwoFloat::from(5.0630000e-001),
                TwoFloat::from(3.7704800e+001),
                TwoFloat::from(-1.2799300e+001),
                TwoFloat::from(-3.5168800e+001),
                TwoFloat::from(-5.5862300e+001),
                TwoFloat::from(-5.5182300e+001),
                TwoFloat::from(3.2800100e+001),
                TwoFloat::from(-3.5502400e+001),
                TwoFloat::from(7.5012000e+000),
                TwoFloat::from(-6.2842800e+001),
                TwoFloat::from(3.5621700e+001),
                TwoFloat::from(-2.1892800e+001),
                TwoFloat::from(6.4802000e+001),
                TwoFloat::from(6.3657900e+001),
                TwoFloat::from(1.6841300e+001),
                TwoFloat::from(-6.2050000e-001),
                TwoFloat::from(7.1958400e+001),
                TwoFloat::from(5.7893200e+001),
                TwoFloat::from(2.6083800e+001),
                TwoFloat::from(5.7235300e+001),
                TwoFloat::from(2.8840900e+001),
                TwoFloat::from(-2.8445200e+001),
                TwoFloat::from(-3.7849300e+001),
                TwoFloat::from(-2.8585100e+001),
                TwoFloat::from(6.1342000e+000),
                TwoFloat::from(4.0880300e+001),
                TwoFloat::from(-3.4327700e+001),
                TwoFloat::from(6.0929200e+001),
                TwoFloat::from(1.2253000e+001),
                TwoFloat::from(-2.3325500e+001),
                TwoFloat::from(3.6493100e+001),
                TwoFloat::from(8.3828000e+000),
                TwoFloat::from(-9.9215000e+000),
                TwoFloat::from(3.5022100e+001),
                TwoFloat::from(2.1835800e+001),
                TwoFloat::from(5.3067700e+001),
                TwoFloat::from(8.2231800e+001),
                TwoFloat::from(4.0662000e+000),
                TwoFloat::from(6.8425500e+001),
                TwoFloat::from(-5.8867800e+001),
                TwoFloat::from(8.6354400e+001),
                TwoFloat::from(-4.1139400e+001),
                TwoFloat::from(-4.4580700e+001),
                TwoFloat::from(6.7633500e+001),
                TwoFloat::from(4.2715000e+001),
                TwoFloat::from(-6.5426600e+001),
                TwoFloat::from(-8.7883700e+001),
                TwoFloat::from(7.0901600e+001),
                TwoFloat::from(-5.4155100e+001),
                TwoFloat::from(-3.6229800e+001),
                TwoFloat::from(2.9059600e+001),
                TwoFloat::from(-3.8806400e+001),
                TwoFloat::from(-5.5396000e+000),
                TwoFloat::from(-7.8339300e+001),
                TwoFloat::from(8.7900200e+001)
            ];
            let bias = 390.0;
