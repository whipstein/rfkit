...
#[derive(Debug, Clone)]
pub struct BracketResult<T: RealScalar> {
...
impl<T: RealScalar> BracketResult<T> {
...
#[derive(Debug, Clone)]
pub struct BracketOptions<T: RealScalar> {
    pub initial_step: T,
    pub max_iters: usize,
    pub tol: T,
...
impl<T: RealScalar> Default for BracketOptions<T> {
    fn default() -> Self {
        Self {
            initial_step: T::ONE,
            max_iters: 100,
...
pub struct Bracket<T: RealScalar> {
    pub a: T,
    pub b: T,
    pub c: T,
    pub fa: T,
...
impl<T: RealScalar> Clone for Bracket<T> {
    fn clone(&self) -> Self {
        Bracket {
            a: self.a.clone(),
            b: self.b.clone(),
...
impl<T: RealScalar> Bracket<T> {
    ...
    pub fn bracket_minimum(
        ...
        while fb > fc && self.iters < max_iter {
            ...
            let val = if q_minus_r != 0.0 {
                q_minus_r.abs().max(tiny.into()) * if q_minus_r > 0.0 { T::ONE } else { -T::ONE }
            } else {
                tiny.into()
            };
            let denom = val * 2.0;
    ...
    pub fn bracket_minimum_adaptive(
        ...
        for &factor in &step_factors {
            ...
            for &direction in &[1.0, -1.0] {
                let dir_step = current_step * direction;
...
impl<T: RealScalar> fmt::Debug for Bracket<T> {
