...
#[derive(Debug, Clone)]
pub struct CmaEsResult<T: RealScalar> {
    xmin: Points1<T>,
    fmin: T,
    generations: usize,
    fn_evals: usize,
...
impl<T: RealScalar> CmaEsResult<T> {
...
impl<T: RealScalar> Clone for CmaEs<T> {
    fn clone(&self) -> Self {
        Self {
...
impl<T: RealScalar + for<'a> std::iter::Sum<&'a T>> CmaEs<T> {
    ...
    fn eigen_decomposition(&self, matrix: &Points2<T>) -> (Points1<T>, Points2<T>) {
        let n = matrix.nrows();
        let mut a = matrix.to_owned();
        let mut v = Points2::<T>::eye(n);
        ...
        for _ in 0..max_iter {
            ...
            let theta = if (a[[q, q]] - a[[p, p]]).abs() < 1e-10 {
                ...
                (a[[p, q]] * 2.0 / (a[[q, q]] - a[[p, p]])).atan() * 0.5
            };
    ...
    pub fn cma_es(
        ...
        let cmu: T = T::from_f64(2.0).min(T::one() - c1) * (mu as f64 - 2.0 + 1.0 / mu as f64)
            / ((n as f64 + 2.0).powi(2) + mu as f64);
        let damps = cs + (1.0 + 2.0 * (0.0_f64).max((mu as f64 - 1.0) / (n as f64 + 1.0) - 1.0));
        ...
        let mut weights = Array1::<T>::from_shape_fn(mu, |i| {
            T::from_f64((mu as f64 + 1.0) / 2.0).ln() - ((i + 1) as f64).ln()
        });
        ...
        while self.generations < max_gen {
            ...
            let mut ymean = Points1::<T>::zeros(n);
            let mut zmean = Points1::<T>::zeros(n);
            for i in 0..mu {
                let idx = indices[i];
                for j in 0..n {
            ...
            for i in 0..n {
                ps[i] = (-cs + 1.0) * ps[i] + (cs * (-cs + 2.0) * mueff).sqrt() * zmean[i];
            }

            let ps_norm = ps.iter().map(|x| x.powi(2)).sum::<T>().sqrt();
            let hsig = if ps_norm / (-(-cs + 1.0).powi(2 * self.generations as i32) + 1.0).sqrt()
                < chi_n * (1.4 + 2.0 / (n as f64 + 1.0))
            {
                T::one()
            } else {
                T::zero()
            ...
            for i in 0..n {
                pc[i] = (-cc + 1.0) * pc[i] + hsig * (cc * (-cc + 2.0) * mueff).sqrt() * ymean[i];
            }

            // Update covariance matrix c
            let c1a = c1 * (-(-hsig.powi(2) + 1.0) * cc * (-cc + 2.0) + 1.0);

            for i in 0..n {
                for j in 0..n {
                    c[[i, j]] = (-c1a - cmu + 1.0) * c[[i, j]] + c1 * pc[i] * pc[j];
...
impl<T: RealScalar> fmt::Debug for CmaEs<T> {
...
mod minimize_cmaes_tests {
    ...
    mod basic_function_tests {
        ...
        fn test_1d_quadratic() {
            ...
            let result = cmaes
                .minimize(&points![twofloat::consts::ZERO], None)
                .unwrap();
    ...
    mod error_handling_tests {
        ...
        fn test_invalid_dimension() {
            let func = |_: &Points1<TwoFloat>| twofloat::consts::ZERO;
            let objective = MultiDimFn::new(func);
            let mut cmaes = CmaEs::new(objective);

            let result = cmaes.minimize(&points![], None);
        ...
        fn test_function_evaluation_error() {
            ...
            let result = cmaes.cma_es(
                &points![10.0.into()], // Start in bad region
                None,
                Some(twofloat::consts::ONE),
                Some(10),
                Some(1e-6.into()),
    ...
    mod diagnostics_and_convergence_tests {
        ...
        fn test_convergence_tracking() {
            ...
            let result = cmaes
                .minimize(&points![2.0.into(), 2.0.into()], None)
                .unwrap();
        ...
        fn test_condition_number_tracking() {
            ...
            let result = cmaes
                .minimize(&points![twofloat::consts::ONE, twofloat::consts::ONE], None)
                .unwrap();
    ...
    mod cec2005_tests {
        ...
        fn test_f1() {
            ...
            // 10d
            let n = 10;
            let x: Points1<TwoFloat> =
                Points(Array1::random(n, Uniform::new(-100_f64, 100_f64).unwrap()))
                    * twofloat::consts::ONE;
        ...
        //     // 10d
        //     let n = 10;
        //     let x = Points1::from_shape_fn(n, |_| (rng.random::<f64>() * 200.0 - 100.0).into());
        //     let mut cmaes =
    ...
    mod utility_and_interface_tests {
        ...
        fn test_get_best_method() {
            ...
            assert_eq!(initial_f, TwoFloat::INFINITY);
        ...
        fn test_debug_formatting() {
            ...
            let _result = cmaes
                .minimize(&points![twofloat::consts::ONE], None)
                .unwrap();
