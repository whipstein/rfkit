...
#[derive(Debug, Clone)]
pub struct BrentResult<T: RealScalar> {
    xmin: T,
    fmin: T,
    fn_evals: usize,
    iters: usize,
...
impl<T: RealScalar> BrentResult<T> {
    pub fn xmin(&self) -> T {
        self.xmin
    }
...
impl<T: RealScalar> Clone for Brent<T> {
    fn clone(&self) -> Self {
        Brent {
            xmin: self.xmin,
            fmin: self.fmin,
...
impl<T: RealScalar> Brent<T> {
    ...
    pub fn line_search(
        ...
        while evaluations < max_evaluations {
            ...
            let tol2_x = tol1_x * 2.0;

            if (x - xm).abs() <= tol2_x - (b - a) * 0.5 {
                break;
            }
            ...
            if e.abs() > tol1_x {
                ...
                if q_final != T::ZERO {
                    ...
                    if q_final > T::ZERO && p > T::ZERO && p_over_q > (a - x) && p_over_q < (b - x)
                    {
                        e = d;
                        d = p_over_q;
                        u = x + d;
    ...
    pub fn minimize_bracket(
        ...
        self.minimize(ax, bx, Some(tol), Some(max_iter))
    }
    ...
    pub fn minimize(
        ...
        while fb.abs() > tol && (bx - ax).abs() > tol && self.iters < max_iter {
            ...
            let condition2 = mflag && (s - bx).abs() >= (bx - c).abs() / 2.0;
            let condition3 = !mflag && (s - bx).abs() >= (c - d).abs() / 2.0;
    ...
    pub fn find_all_roots(
        ...
        for i in 0..n_sub {
            let x1 = a + dx * i as f64;
            let x2 = a + dx * (i + 1) as f64;
            ...
            if f1 * f2 < 0.0 {
                if let Ok(result) = self.minimize(x1, x2, Some(tol), None) {
                    ...
                    let is_unique = roots.iter().all(|existing_root: &T| {
                        (-*existing_root + result.xmin).abs() > tol * 10.0
                    });
...
impl<T: RealScalar> fmt::Debug for Brent<T> {
...
mod minimize_brent_tests {
    use super::*;
    use crate::minimize::SingleDimFn;
    use ndarray::prelude::*;
    ...
    const TIGHT_MARGIN: NumMargin<f64> = NumMargin {
        epsilon: 1e-10,
        relative: 1e-10,
        ulps: 4,
    };
    ...
    mod root_finding_tests {
        ...
        #[test]
        fn test_linear_root() {
            let f = |x: &TwoFloat| 2.0 * x - 6.0; // Root at x = 3
            let objective = SingleDimFn::new(f);
            let mut brent = Brent::new(objective);

            let result = brent.find_root(0.0.into(), 5.0.into()).unwrap();

            result.xmin.assert_approx_eq(
                &TwoFloat::from(3.0),
                TIGHT_MARGIN.into(),
                "linear_root",
                "xmin",
        ...
        fn test_cubic_multiple_roots() {
            ...
            let f = |x: &TwoFloat| x.powi(3) - 6.0 * x.powi(2) + 11.0 * x - 6.0;
            let objective = SingleDimFn::new(f);
    ...
    mod transcendental_function_tests {
        ...
        fn test_exponential_root() {
            ...
            let f = |x: &TwoFloat| x.exp() - 2.0;
            let objective = SingleDimFn::new(f);
            let mut brent = Brent::new(objective);

            let result = brent.find_root(0.0.into(), 1.0.into()).unwrap();
            let expected = TwoFloat::from(2.0).ln();
    ...
    mod edge_case_and_robustness_tests {
        ...
        fn test_very_large_bracket() {
            let f = |x: &TwoFloat| x - 1000.0;
            let objective = SingleDimFn::new(f);
            let mut brent = Brent::new(objective);

            let result = brent.find_root((-1e6).into(), 1e6.into()).unwrap();
            result.xmin.assert_approx_eq(
                &TwoFloat::from(1000.0),
                LOOSE_MARGIN.into(),
                "very_large_bracket",
                "xmin",
            );
        ...
        fn test_oscillatory_function() {
            ...
            let f = |x: &TwoFloat| x - 1.0 + 0.01 * (100.0 * x).sin();
            let objective = SingleDimFn::new(f);
            let mut brent = Brent::new(objective);
    ...
    mod tolerance_and_precision_tests {
        ...
        fn test_default_parameters() {
            let f = |x: &TwoFloat| x - 3.14;
            let objective = SingleDimFn::new(f);
            let mut brent = Brent::new(objective);

            let result = brent.minimize(3.0.into(), 3.2.into(), None, None).unwrap();
            result.xmin.assert_approx_eq(
                &TwoFloat::from(3.14),
                TIGHT_MARGIN.into(),
                "default_parameters",
    ...
    mod find_all_roots_tests {
        ...
        fn test_find_all_roots_polynomial() {
            ...
            let f = |x: &TwoFloat| x.powi(3) - 2.0 * x.powi(2) - 5.0 * x + 6.0;
            let objective = SingleDimFn::new(f);
            let mut brent = Brent::new(objective);

            let roots =
                brent.find_all_roots((-3.0).into(), 4.0.into(), Some(100), Some(1e-10.into()));

            assert_eq!(roots.len(), 3);
            assert!(
                roots
                    .iter()
                    .any(|r| r.approx_eq(&TwoFloat::from(-2.0), TIGHT_MARGIN.into()))
            );
            assert!(
                roots
                    .iter()
                    .any(|r| r.approx_eq(&twofloat::consts::ONE, TIGHT_MARGIN.into()))
            );
            assert!(
                roots
                    .iter()
                    .any(|r| r.approx_eq(&TwoFloat::from(3.0), TIGHT_MARGIN.into()))
            );
        }
        ...
        fn test_find_all_roots_trigonometric() {
            ...
            assert!(
                roots
                    .iter()
                    .any(|r| r.approx_eq(&TwoFloat::from(PI), TIGHT_MARGIN.into()))
            );
        }

        #[test]
        fn test_find_all_roots_no_roots() {
            let f = |x: &TwoFloat| x * x + 1.0; // No real roots
            let objective = SingleDimFn::new(f);
            let mut brent = Brent::new(objective);

            let roots = brent.find_all_roots((-2.0).into(), 2.0.into(), Some(100), None);
            assert!(roots.is_empty());
        }

        #[test]
        fn test_find_all_roots_single_root() {
            let f = |x: &TwoFloat| (x - 2.5).powi(2); // Double root at x = 2.5
            let objective = SingleDimFn::new(f);
            let mut brent = Brent::new(objective);

            // This is tricky because there's no sign change, but we test boundary
            let _ =
                brent.find_all_roots(2.49999.into(), 2.50001.into(), Some(10), Some(1e-10.into()));
    ...
    mod line_search_tests {
        ...
        fn test_line_search_basic() {
            let f = |x: &TwoFloat| x * x; // Minimum at x = 0
            let objective = SingleDimFn::new(f);
            let mut brent = Brent::new(objective);

            let point = array![1.0.into()].into();
            let direction = array![TwoFloat::from_f64(-1.0)].into(); // Search towards x = 0
        ...
        fn test_line_search_multidimensional_context() {
            ...
            let f = |x: &TwoFloat| x * x + 1.0; // Simple 1D function for testing
            let objective = SingleDimFn::new(f);
            let mut brent = Brent::new(objective);

            let point = array![2.0.into(), 3.0.into()].into(); // Starting point in 2D
            let direction = array![TwoFloat::from_f64(-1.0), 0.0.into()].into(); // Search in x-direction only
    ...
    mod bracket_tests {
        ...
        fn test_minimize_bracket() {
            let f = |x: &TwoFloat| (x - 3.0) * (x + 1.0); // Root at x = 3 and x = -1
            let objective = SingleDimFn::new(f);
            let mut brent = Brent::new(objective);
            ...
            match result {
                Ok(res) => {
                    assert!(
                        res.xmin
                            .approx_eq(&TwoFloat::from(3.0), LOOSE_MARGIN.into())
                            || res
                                .xmin
                                .approx_eq(&TwoFloat::from(-1.0), LOOSE_MARGIN.into())
                    );
                }
                Err(_) => {
    ...
    mod special_value_tests {
        ...
        fn test_function_with_zero_derivative() {
            ...
            result.xmin.assert_approx_eq(
                &twofloat::consts::ONE,
                LOOSE_MARGIN.into(),
                "function_with_zero_derivatives",
                "xmin",
            );
        }
        ...
        fn test_function_with_inflection_point() {
            ...
            let f = |x: &TwoFloat| x.powi(3);
            let objective = SingleDimFn::new(f);
            let mut brent = Brent::new(objective);

            let result = brent.find_root((-1.0).into(), 1.0.into()).unwrap();
            result.xmin.assert_approx_eq(
                &twofloat::consts::ZERO,
                TIGHT_MARGIN.into(),
                "function_with_inflection_point",
                "xmin",
            );
        }
    }
    ...
    mod debug_and_display_tests {
        ...
        fn test_debug_formatting() {
            let f = |x: &TwoFloat| x - 1.0; // Root at x = 1
            let objective = SingleDimFn::new(f);
            let mut brent = Brent::new(objective);
    ...
    mod stress_tests {
        ...
        fn test_many_roots_interval() {
            ...
            let f = |x: &TwoFloat| (10.0 * x).sin();
            let objective = SingleDimFn::new(f);
            let mut brent = Brent::new(objective);

            let roots = brent.find_all_roots(0.1.into(), 3.0.into(), Some(500), Some(1e-8.into()));
        ...
        fn test_pathological_bracket_size() {
            ...
            let result = brent.find_root((-1e-10).into(), 1e-10.into()).unwrap();
