...
#[derive(Debug, Clone)]
pub struct ConjGradResult<T: RealScalar> {
    xmin: Points1<T>,
    fmin: T,
    gradient_norm: T,
    iters: usize,
...
impl<T: RealScalar> ConjGradResult<T> {
...
pub struct ConjGrad<T: RealScalar> {
    xmin: Points1<T>,
    fmin: T,
...
impl<T: RealScalar> Clone for ConjGrad<T> {
    fn clone(&self) -> Self {
        Self {
            ...
            let beta = match method {
                ...
                ConjGradMethod::HagerZhang => {
                    ...
                    if hd_dot.abs() < 1e-12 {
                        ...
                        let scaling = grad_diff_norm_sq * 2.0 / hd_dot;
...
impl<T: RealScalar> fmt::Debug for ConjGrad<T> {
...
mod minimize_conjgrad_tests {
    ...
    fn test_2d_quadratic() {
        ...
        let grad = |x: &Points1<TwoFloat>| points![(x[0] - 1.0) * 2.0, (x[1] - 2.0) * 2.0];
        let obj = MultiDimGradFn::new(func, grad);
        let mut conjgrad = ConjGrad::new(obj);

        let result = conjgrad
            .minimize(&points![twofloat::consts::ZERO, twofloat::consts::ZERO])
            .unwrap();
    ...
    fn test_rosenbrock() {
        ...
        let result = conjgrad
            .conjugate_gradient(
                &points![TwoFloat::from_f64(-1.2), twofloat::consts::ONE],
                ConjGradMethod::PolakRibiere,
    ...
    fn test_different_methods() {
        ...
        let grad = |x: &Points1<TwoFloat>| points![2.0 * x[0] + x[1], 4.0 * x[1] + x[0]];
        let obj = MultiDimGradFn::new(func, grad);
        let mut conjgrad = ConjGrad::new(obj);
        ...
        for method in &methods {
            ...
            match result {
                ...
                Err(_) => {
                    ...
                    let simple_grad = |x: &Points1<TwoFloat>| points![2.0 * x[0], 2.0 * x[1]];
    ...
    fn test_method_comparison() {
        ...
        let results = conjgrad.compare_cg_methods(
            &points![twofloat::consts::ONE, 2.0.into(), 3.0.into()],
            Some(1e-8.into()),
            Some(100),
        );
    ...
    fn test_cg_restart_mechanism() {
        ...
        let ill_conditioned =
            |x: &Points1<TwoFloat>| 1000.0 * x[0].powi(2) + x[1].powi(2) + 0.1 * x[0] * x[1];
        let ill_conditioned_grad =
            |x: &Points1<TwoFloat>| points![2000.0 * x[0] + 0.1 * x[1], 2.0 * x[1] + 0.1 * x[0]];
    ...
    fn test_cg_numerical_gradients() {
        ...
        let objective =
            |x: &Points1<TwoFloat>| (x[0] - 1.0).powi(4) + (x[1] + 2.0).powi(2) + 3.0 * x[0] * x[1];
    ...
    mod basic_functionality_tests {
        ...
        fn test_conjgrad_construction() {
            ...
            assert_eq!(conjgrad.fmin, twofloat::consts::ZERO);
        ...
        fn test_debug_formatting() {
            let obj = simple_quadratic();
            let mut conjgrad = ConjGrad::new(obj);
            let _ = conjgrad.minimize(&points![twofloat::consts::ONE, 2.0.into()]);
    ...
    mod error_handling_tests {
        ...
        fn test_empty_initial_point_error() {
            ...
            let result = conjgrad.minimize(&points![]);
        ...
        fn test_zero_tolerance_error() {
            ...
            let result = conjgrad.conjugate_gradient(
                &points![twofloat::consts::ONE, 2.0.into()],
                ConjGradMethod::FletcherReeves,
                Some(twofloat::consts::ZERO),
                None,
                None,
            );
        ...
        fn test_wrong_gradient_dimension() {
            ...
            let wrong_grad = |_x: &Points1<TwoFloat>| points![twofloat::consts::ONE]; // Wrong dimension
            let obj = MultiDimGradFn::new(func, wrong_grad);
            let mut conjgrad = ConjGrad::new(obj);

            let result = conjgrad.minimize(&points![twofloat::consts::ONE, twofloat::consts::ONE]);
            assert!(matches!(
                result,
    ...
    mod basic_optimization_tests {
        ...
        fn test_shifted_quadratic() {
            let center = points![3.0.into(), TwoFloat::from_f64(-2.0)];
        ...
        fn test_1d_optimization() {
            ...
            let grad = |x: &Points1<TwoFloat>| points![2.0 * (&x[0] - 5.0)];
            let obj = MultiDimGradFn::new(func, grad);
            let mut conjgrad = ConjGrad::new(obj);

            let result = conjgrad.minimize(&points![twofloat::consts::ZERO]).unwrap();
    ...
    mod cg_method_comparison_tests {
        ...
        fn test_all_cg_methods_simple_quadratic() {
            ...
            for &method in &methods {
                ...
                let result = conjgrad
                    .conjugate_gradient(
                        &points![2.0.into(), TwoFloat::from_f64(-1.5)],
                        method,
                        Some(1e-8.into()),
                        Some(100),
                        None,
        ...
        fn test_fletcher_reeves_method() {
            let obj = shifted_quadratic(&points![twofloat::consts::ONE, TwoFloat::from_f64(-1.0)]);
            let mut conjgrad = ConjGrad::new(obj);
        ...
        fn test_polak_ribiere_method() {
            ...
            let result = conjgrad
                .conjugate_gradient(
                    &points![3.0.into(), TwoFloat::from_f64(-2.0)],
                    ConjGradMethod::PolakRibiere,
                    Some(1e-8.into()),
                    Some(50),
                    None,
    ...
    mod convergence_analysis_tests {
        ...
        fn test_convergence_history_structure() {
            ...
            let result = conjgrad
                .minimize(&points![2.0.into(), TwoFloat::from_f64(-1.0)])
                .unwrap();
        ...
        fn test_evaluation_counts() {
            ...
            let result = conjgrad
                .minimize(&points![twofloat::consts::ONE, 2.0.into()])
                .unwrap();
        ...
        fn test_tolerance_behavior() {
            ...
            for &tol in &tolerances {
                ...
                let result = conjgrad
                    .conjugate_gradient(
                        &points![2.0.into(), TwoFloat::from_f64(-1.0)],
                        ConjGradMethod::PolakRibiere,
                        Some(tol.into()),
                        Some(100),
                        None,
    ...
    mod restart_mechanism_tests {
        ...
        fn test_restart_functionality() {
            ...
            let grad = |x: &Points1<TwoFloat>| points![200.0 * &x[0], 2.0 * &x[1]];
            let obj = MultiDimGradFn::new(func, grad);
            let mut conjgrad = ConjGrad::new(obj);
    ...
    mod difficult_function_tests {
        ...
        fn test_booth_function() {
            ...
            let grad = |x: &Points1<TwoFloat>| {
                ...
                points![2.0 * &t1 + 4.0 * &t2, 4.0 * &t1 + 2.0 * &t2]
            };
            let obj = MultiDimGradFn::new(func, grad);
            let mut conjgrad = ConjGrad::new(obj);
    ...
    mod robustness_tests {
        ...
        fn test_large_initial_values() {
            ...
            let result = conjgrad
                .minimize(&points![100.0.into(), TwoFloat::from_f64(-50.0)])
                .unwrap();
        ...
        fn test_function_with_different_scales() {
            ...
            let grad = |x: &Points1<TwoFloat>| points![2000.0 * &x[0], 2.0 * &x[1]];
            let obj = MultiDimGradFn::new(func, grad);
            let mut conjgrad = ConjGrad::new(obj);
    ...
    mod edge_case_tests {
        ...
        fn test_clone_and_debug_traits() {
            let obj = simple_quadratic();
            let mut conjgrad = ConjGrad::new(obj);
            let result = conjgrad.minimize(&points![twofloat::consts::ONE]).unwrap();
    ...
    mod stress_tests {
        ...
        fn test_various_starting_points() {
            let starting_points = vec![
                points![twofloat::consts::ZERO, twofloat::consts::ZERO],
                points![twofloat::consts::ONE, twofloat::consts::ONE],
                points![TwoFloat::from_f64(-1.0), twofloat::consts::ONE],
                points![10.0.into(), TwoFloat::from_f64(-5.0)],
                points![TwoFloat::from_f64(-20.0), 15.0.into()],
            ];

            for start in starting_points {
                let obj = shifted_quadratic(&points![2.0.into(), TwoFloat::from_f64(-1.0)]);
                let mut conjgrad = ConjGrad::new(obj);

                let result = conjgrad.minimize(&start).unwrap();
