...
use crate::{error::MinimizerError, minimize::ObjDerFn};
use rfkit_base::prelude::*;
use std::fmt;

/// Result of Brent's method with derivatives
#[derive(Debug, Clone)]
pub struct DBrentResult<T: RealScalar> {
    xmin: T,
    fmin: T,
    dfmin: T,
    iters: usize,
...
impl<T: RealScalar> DBrentResult<T> {
    pub fn xmin(&self) -> T {
        self.xmin
    }
...
pub struct DBrent<T: RealScalar> {
    xmin: T,
    fmin: T,
    dfmin: T,
...
impl<T: RealScalar> Clone for DBrent<T> {
    fn clone(&self) -> Self {
        DBrent {
            xmin: self.xmin,
            fmin: self.fmin,
...
impl<T: RealScalar> DBrent<T> {
    ...
    pub fn minimize(
        ...
        let mut eval_a = (ax, self.f.call_scalar(&ax), self.f.df_scalar(&ax));
        let mut eval_b = (bx, self.f.call_scalar(&bx), self.f.df_scalar(&bx));
        ...
        while eval_b.1.abs() > tol && (bx - ax).abs() > tol && self.iters < max_iter {
            ...
            let condition1 = !(((ax * 3.0 + bx) / 4.0)..=bx).contains(&s);
...
impl<T: RealScalar> fmt::Debug for DBrent<T> {
...
mod minimize_myfloat_dbrent_tests {
    use super::*;
    use crate::minimize::SingleDimDerFn;
    ...
    const HIGH_PRECISION_MARGIN: NumMargin<f64> = NumMargin {
        epsilon: 1e-12,
        relative: 1e-12,
        ulps: 4,
    };
    const STANDARD_MARGIN: NumMargin<f64> = NumMargin {
        epsilon: 1e-8,
        relative: 1e-8,
        ulps: 10,
    };
    ...
    fn test_quadratic_with_derivative() {
        ...
        let f = |x: &TwoFloat| x * x - 2.0;
        let df = |x: &TwoFloat| 2.0 * x;
    ...
    fn test_cubic_with_derivative() {
        ...
        let f = |x: &TwoFloat| x.powi(3) - 2.0 * x - 5.0;
        let df = |x: &TwoFloat| 3.0 * x * x - 2.0;
        let objective = SingleDimDerFn::new(f, df);
        let mut dbrent = DBrent::new(objective);
    ...
    fn test_transcendental_with_derivative() {
        ...
        let f = |x: &TwoFloat| x.exp() - 2.0 * x - 1.0;
        let df = |x: &TwoFloat| x.exp() - 2.0;
        let objective = SingleDimDerFn::new(f, df);
        let mut dbrent = DBrent::new(objective);
    ...
    fn test_newton_with_bracket() {
        ...
        let f = |x: &TwoFloat| x.powi(3) - x - 1.0;
        let df = |x: &TwoFloat| 3.0 * x * x - 1.0;
        let objective = SingleDimDerFn::new(f, df);
        let mut dbrent = DBrent::new(objective);
    ...
    fn test_method_tracking() {
        let f = |x: &TwoFloat| x.powi(3) - 2.0 * x - 2.0;
        let df = |x: &TwoFloat| 3.0 * x * x - 2.0;
        let objective = SingleDimDerFn::new(f, df);
        let mut dbrent = DBrent::new(objective);
    ...
    fn test_same_sign_error() {
        let f = |x: &TwoFloat| x * x + 1.0; // Always positive
        let df = |x: &TwoFloat| 2.0 * x;
        let objective = SingleDimDerFn::new(f, df);
        let mut dbrent = DBrent::new(objective);
    ...
    fn test_high_precision() {
        ...
        let f = |x: &TwoFloat| x.sin();
        let df = |x: &TwoFloat| x.cos();
        let objective = SingleDimDerFn::new(f, df);
        let mut dbrent = DBrent::new(objective);
    ...
    fn test_brent_method_convergence_speed() {
        ...
        let cubic = |x: &TwoFloat| x.powi(3) - 2.0 * x - 5.0;
        let cubic_grad = |x: &TwoFloat| 3.0 * x.powi(2) - 2.0;
    ...
    fn test_brent_method_difficult_functions() {
        ...
        let flat_derivative = |x: &TwoFloat| x.powi(5) - x;
        let flat_derivative_grad = |x: &TwoFloat| 5.0 * x.powi(4) - 1.0;
        ...
        let oscillatory = |x: &TwoFloat| x * x - 2.0 + 0.1 * (10.0 * x).sin();
        let oscillatory_grad = |x: &TwoFloat| 2.0 * x + (10.0 * x).cos();
    ...
    fn test_newton_with_bracket_robustness() {
        ...
        let difficult = |x: &TwoFloat| (x - 1.0).powi(3);
        let difficult_grad = |x: &TwoFloat| 3.0 * (x - 1.0).powi(2);
        let objective = SingleDimDerFn::new(difficult, difficult_grad);
        let mut dbrent = DBrent::new(objective);

        let result = dbrent
            .newton_with_bracket(2.0.into(), 0.5.into(), 1.5.into(), Some(1e-10.into()), None)
            .unwrap();
    ...
    mod basic_functionality_tests {
        ...
        fn test_linear_function() {
            ...
            let f = |x: &TwoFloat| 2.0 * x - 4.0;
            let df = |_x: &TwoFloat| 2.0.into();
            let objective = SingleDimDerFn::new(f, df);
            let mut dbrent = DBrent::new(objective);
        ...
        fn test_simple_quadratic() {
            ...
            let f = |x: &TwoFloat| (x - 3.0).powi(2) - 4.0;
            let df = |x: &TwoFloat| 2.0 * (x - 3.0);
            let objective = SingleDimDerFn::new(f, df);
            let mut dbrent = DBrent::new(objective);
        ...
        fn test_cubic_multiple_roots() {
            ...
            // f'(x) = 3x^2 - 4x - 1
            // Roots at x = -1, 1, 2
            let f = |x: &TwoFloat| x.powi(3) - 2.0 * x.powi(2) - x + 2.0;
            let df = |x: &TwoFloat| 3.0 * x.powi(2) - 4.0 * x - 1.0;
            let objective = SingleDimDerFn::new(f, df);
            let mut dbrent = DBrent::new(objective);

            // Test root at x = -1
            let result = dbrent
                .find_root_with_derivative((-2.0).into(), 0.0.into())
                .unwrap();
    ...
    mod transcendental_functions_tests {
        ...
        fn test_exponential_function() {
            ...
            let f = |x: &TwoFloat| x.exp() - 3.0 * x - 1.0;
            let df = |x: &TwoFloat| x.exp() - 3.0;
            let objective = SingleDimDerFn::new(f, df);
            let mut dbrent = DBrent::new(objective);
        ...
        fn test_logarithmic_function() {
            ...
            let f = |x: &TwoFloat| x.ln() - 1.0;
            let df = |x: &TwoFloat| 1.0 / x;
            let objective = SingleDimDerFn::new(f, df);
            let mut dbrent = DBrent::new(objective);
    ...
    mod challenging_numerical_cases {
        ...
        fn test_nearly_flat_function() {
            ...
            let f = |x: &TwoFloat| x.powi(7) - 0.1;
            let df = |x: &TwoFloat| 7.0 * x.powi(6);
            let objective = SingleDimDerFn::new(f, df);
            let mut dbrent = DBrent::new(objective);
            ...
            let expected_root = TwoFloat::from(0.1).powf(TwoFloat::from(7.0).recip());
        ...
        fn test_steep_function() {
            ...
            let f = |x: &TwoFloat| x.powf(TwoFloat::from(7.0).recip()) - 2.0;
            let df =
                |x: &TwoFloat| (TwoFloat::from(7.0).recip()) * x.powf(TwoFloat::from(-6.0) / 7.0);
            let objective = SingleDimDerFn::new(f, df);
            let mut dbrent = DBrent::new(objective);
            ...
            let expected_root = TwoFloat::from_f64(2.0).powi(7); // 128
        ...
        fn test_oscillatory_function() {
            ...
            let f = |x: &TwoFloat| x - 1.0 + 0.5 * (10.0 * x).sin();
            let df = |x: &TwoFloat| 1.0 + 5.0 * (10.0 * x).cos();
            let objective = SingleDimDerFn::new(f, df);
            let mut dbrent = DBrent::new(objective);
        ...
        fn test_multiple_inflection_points() {
            ...
            let f = |x: &TwoFloat| x.powi(4) - 8.0 * x.powi(3) + 18.0 * x.powi(2) - 8.0 * x - 3.0;
            let df = |x: &TwoFloat| 4.0 * x.powi(3) - 24.0 * x.powi(2) + 36.0 * x - 8.0;
            let objective = SingleDimDerFn::new(f, df);
            let mut dbrent = DBrent::new(objective);
    ...
    mod precision_and_tolerance_tests {
        ...
        fn test_loose_tolerance() {
            let f = |x: &TwoFloat| x.powi(3) - x - 1.0;
            let df = |x: &TwoFloat| 3.0 * x.powi(2) - 1.0;
            let objective = SingleDimDerFn::new(f, df);
            let mut dbrent = DBrent::new(objective);
    ...
    mod newton_with_bracket_tests {
        ...
        fn test_newton_bracket_inside_interval() {
            ...
            let f = |x: &TwoFloat| x.powi(2) - 4.0;
            let df = |x: &TwoFloat| 2.0 * x;
            let objective = SingleDimDerFn::new(f, df);
            let mut dbrent = DBrent::new(objective);

            let result = dbrent
                .newton_with_bracket(2.1.into(), 1.8.into(), 2.3.into(), Some(1e-12.into()), None)
                .unwrap();
        ...
        fn test_newton_bracket_fallback_to_bisection() {
            ...
            let f = |x: &TwoFloat| x - 1.0; // Simple linear function with root at x = 1
            let df = |_x: &TwoFloat| 1.0.into();
            let objective = SingleDimDerFn::new(f, df);
            let mut dbrent = DBrent::new(objective);
        ...
        fn test_newton_bracket_zero_derivative() {
            ...
            let f = |x: &TwoFloat| (x - 1.0).powi(3) + 0.001;
            let df = |x: &TwoFloat| 3.0 * (x - 1.0).powi(2);
            let objective = SingleDimDerFn::new(f, df);
            let mut dbrent = DBrent::new(objective);

            // Start near where derivative is zero
            let result = dbrent
                .newton_with_bracket(1.01.into(), 0.5.into(), 1.5.into(), Some(1e-8.into()), None)
                .unwrap();
    ...
    mod error_condition_tests {
        ...
        fn test_same_sign_at_endpoints() {
            ...
            let f = |x: &TwoFloat| x.powi(2) + 1.0; // Always positive
            let df = |x: &TwoFloat| 2.0 * x;
            let objective = SingleDimDerFn::new(f, df);
            let mut dbrent = DBrent::new(objective);

            let result = dbrent.find_root_with_derivative(TwoFloat::from_f64(-1.0), 1.0.into());
            assert!(matches!(result, Err(MinimizerError::SameSignError)));
        }
        ...
        fn test_max_iterations_exceeded() {
            ...
            let f = |x: &TwoFloat| (x - 1.0) * 1e-10; // Very small slope
            let df = |_x: &TwoFloat| 1e-10.into();
            let objective = SingleDimDerFn::new(f, df);
            let mut dbrent = DBrent::new(objective);
    ...
    mod method_selection_and_tracking_tests {
        ...
        fn test_method_usage_tracking() {
            ...
            let f = |x: &TwoFloat| x.powi(3) - 2.0 * x - 5.0; // Has root around x = 2.09
            let df = |x: &TwoFloat| 3.0 * x.powi(2) - 2.0;
            let objective = SingleDimDerFn::new(f, df);
            let mut dbrent = DBrent::new(objective);
        ...
        fn test_preferential_newton_usage() {
            ...
            let f = |x: &TwoFloat| x.powi(2) - 4.0; // Root at x = 2, simple and well-behaved
            let df = |x: &TwoFloat| 2.0 * x;
            let objective = SingleDimDerFn::new(f, df);
            let mut dbrent = DBrent::new(objective);
        ...
        fn test_bisection_safety_fallback() {
            ...
            let f = |x: &TwoFloat| x.atan() - 0.5; // Moderately nonlinear
            let df = |x: &TwoFloat| 1.0 / (1.0 + x.powi(2));
            let objective = SingleDimDerFn::new(f, df);
            let mut dbrent = DBrent::new(objective);
    ...
    mod boundary_and_edge_cases {
        ...
        fn test_root_at_bracket_boundary() {
            let f = |x: &TwoFloat| x - 1.0; // Root exactly at x = 1
            let df = |_x: &TwoFloat| 1.0.into();
            let objective = SingleDimDerFn::new(f, df);
            let mut dbrent = DBrent::new(objective);
    ...
    mod additional_robustness_tests {
        ...
        fn test_function_with_multiple_scales() {
            ...
            let f = |x: &TwoFloat| x - 1.0 + 0.1 * (x - 1.0).sin(); // Root near x = 1
            let df = |x: &TwoFloat| 1.0 + 0.1 * (x - 1.0).cos();
            let objective = SingleDimDerFn::new(f, df);
            let mut dbrent = DBrent::new(objective);
        ...
        fn test_near_machine_precision() {
            ...
            let f = |x: &TwoFloat| x - 1.0 - f64::EPSILON;
            let df = |_x: &TwoFloat| 1.0.into();
            let objective = SingleDimDerFn::new(f, df);
            let mut dbrent = DBrent::new(objective);
        ...
        fn test_discontinuous_derivative() {
            ...
            let f = |x: &TwoFloat| if *x >= 0.0 { x - 0.5 } else { -x - 0.5 }; // Root at x = 0.5 and x = -0.5
            let df = |x: &TwoFloat| {
                if *x >= 0.0 {
                    1.0.into()
                } else {
        ...
        fn test_very_large_function_values() {
            ...
            let f = |x: &TwoFloat| 1e12 * (x - 1.0);
            let df = |_x: &TwoFloat| 1e12.into();
            let objective = SingleDimDerFn::new(f, df);
            let mut dbrent = DBrent::new(objective);
        ...
        fn test_very_small_function_values() {
            ...
            let f = |x: &TwoFloat| 1e-12 * (x - 1.0);
            let df = |_x: &TwoFloat| 1e-12.into();
            let objective = SingleDimDerFn::new(f, df);
            let mut dbrent = DBrent::new(objective);
        ...
        fn test_asymptotic_behavior() {
            ...
            let f = |x: &TwoFloat| 1.0 / x - 1.0; // Root at x = 1
            let df = |x: &TwoFloat| -1.0 / (x * x);
            let objective = SingleDimDerFn::new(f, df);
            let mut dbrent = DBrent::new(objective);
    ...
    mod advanced_method_selection_tests {
        ...
        fn test_inverse_quadratic_interpolation_usage() {
            ...
            let f = |x: &TwoFloat| (x - 1.0) * (x - 2.0) * (x - 3.0);
            let df = |x: &TwoFloat| 3.0 * x.powi(2) - 12.0 * x + 11.0;
            let objective = SingleDimDerFn::new(f, df);
            let mut dbrent = DBrent::new(objective);
        ...
        fn test_secant_method_fallback() {
            ...
            let f = |x: &TwoFloat| x.powi(2) - x - 1.0; // Quadratic with reasonable behavior
            let df = |x: &TwoFloat| 2.0 * x - 1.0;
            let objective = SingleDimDerFn::new(f, df);
            let mut dbrent = DBrent::new(objective);
            ...
            let expected_root = (1.0 + TwoFloat::from_f64(5.0).sqrt()) / 2.0; // Golden ratio
    ...
    mod comperhensive_error_condition_tests {
        ...
        fn test_all_error_conditions_systematically() {
            ...
            let positive_f = |x: &TwoFloat| x.powi(2) + 1.0;
            let positive_df = |x: &TwoFloat| 2.0 * x;
            let pos_obj = SingleDimDerFn::new(positive_f, positive_df);
            let mut pos_dbrent = DBrent::new(pos_obj);

            assert!(matches!(
                pos_dbrent.minimize(TwoFloat::from_f64(-1.0), 1.0.into(), None, None),
                Err(MinimizerError::SameSignError)
            ));

            // Test max iterations exceeded - use a challenging but solvable function
            let slow_f = |x: &TwoFloat| (x - 1.0) * 1e-12; // Extremely small gradient
            let slow_df = |_x: &TwoFloat| 1e-12.into();
        ...
        fn test_numerical_edge_cases_comprehensive() {
            ...
            let inflection = |x: &TwoFloat| (x - 1.0).powi(3) + 0.001 * (x - 1.0); // Small linear term to ensure root exists
            let dinflection = |x: &TwoFloat| 3.0 * (x - 1.0).powi(2) + 0.001;
            ...
            let sharp = |x: &TwoFloat| (x - 1.0).atan() * 100.0; // Reduced scale for numerical stability
            let dsharp = |x: &TwoFloat| 100.0 / (1.0 + (x - 1.0).powi(2));
            ...
            let flat = |x: &TwoFloat| (x - 1.0) + 1e-6 * (x - 1.0).powi(3);
            let dflat = |x: &TwoFloat| 1.0 + 3e-6 * (x - 1.0).powi(2);
    ...
    mod performance_and_convergence_tests {
        ...
        fn test_convergence_speed_comparison() {
            ...
            let smooth = |x: &TwoFloat| (x - 1.5).powi(2) - 0.25; // Root at x = 1.0 and x = 2.0
            let dsmooth = |x: &TwoFloat| 2.0 * (x - 1.5);
            ...
            let moderate = |x: &TwoFloat| x.powi(3) - x - 1.0;
            let dmoderate = |x: &TwoFloat| 3.0 * x.powi(2) - 1.0;
        ...
        fn test_pathological_derivative() {
            ...
            let f = |x: &TwoFloat| (x - 1.0).atan() * 1000.0;
            let df = |x: &TwoFloat| 1000.0 / (1.0 + (x - 1.0).powi(2));
            let objective = SingleDimDerFn::new(f, df);
            let mut dbrent = DBrent::new(objective);
        ...
        fn test_integration_with_different_function_types() {
            ...
            // Polynomial
            let poly = |x: &TwoFloat| x.powi(4) - 10.0 * x.powi(2) + 9.0; // (x²-1)(x²-9)
            let dpoly = |x: &TwoFloat| 4.0 * x.powi(3) - 20.0 * x;
            ...
            // Rational
            let rational = |x: &TwoFloat| (x - 1.0) / (x.powi(2) + 1.0);
            let drational = |x: &TwoFloat| (1.0 - x.powi(2) + 2.0 * x) / (x.powi(2) + 1.0).powi(2);
        ...
        fn test_method_efficiency_analysis() {
            ...
            let analyze = |x: &TwoFloat| x.exp() - 3.0 * x - 1.0;
            let danalyze = |x: &TwoFloat| x.exp() - 3.0;
