...
use crate::{error::MinimizerError, minimize::ObjFn};
use rfkit_base::prelude::*;
use std::fmt;

/// Result of golden section search
#[derive(Debug, Clone)]
pub struct GoldenResult<T: RealScalar> {
    xmin: T,
    fmin: T,
    iters: usize,
    converged: bool,
...
impl<T: RealScalar> GoldenResult<T> {
    pub fn xmin(&self) -> T {
        self.xmin
    }
...
pub struct Golden<T: RealScalar> {
    xmin: T,
    fmin: T,
    f: Box<dyn ObjFn<T>>,
    iters: usize,
...
impl<T: RealScalar> Clone for Golden<T> {
    fn clone(&self) -> Self {
        Self {
            xmin: self.xmin,
            fmin: self.fmin,
...
impl<T: RealScalar> Golden<T> {
    ...
    pub fn golden_section_search(
        ...
        let mut x2 = x1 + (x4 - x1) * (1.0 - Golden::<T>::GOLDEN_RATIO);
        let mut x3 = x1 + (x4 - x1) * Golden::<T>::GOLDEN_RATIO;
        ...
        while (x4 - x1).abs() > tol && self.iters < max_iter {
            ...
            if f2 < f3 {
                ...
                x3 = x2;
                f3 = f2;
                x2 = x1 + (x4 - x1) * (1.0 - Golden::<T>::GOLDEN_RATIO);
                ...
                x2 = x3;
                f2 = f3;
                x3 = x1 + (x4 - x1) * Golden::<T>::GOLDEN_RATIO;
...
impl<T: RealScalar> fmt::Debug for Golden<T> {
...
mod minimize_golden_tests {
    use super::*;
    use crate::minimize::{F1dim, SingleDimFn};
    ...
    fn test_golden_section_unimodal_functions() {
        ...
        let quadratic = |x: &TwoFloat| (x - 3.0).powi(2) + 2.0;
        ...
        let quartic = |x: &TwoFloat| (x - 1.5).powi(4) + 0.5;
        ...
        let transcendental = |x: &TwoFloat| x * x + (2.0 * x).sin();
    ...
    fn test_golden_section_edge_cases() {
        ...
        let func = |x: &TwoFloat| (x - 0.5).powi(2);
        ...
        let difficult = |x: &TwoFloat| (x - PI).powi(2) + 1e-10 * (100.0 * x).sin();
    ...
    mod basic_functionality_tests {
        ...
        fn test_boxed_construction() {
            let f = |x: &TwoFloat| x * x;
            let objective = SingleDimFn::new(f);
            let boxed_obj: Box<dyn ObjFn<TwoFloat>> = Box::new(objective);
    ...
    mod mathematical_function_tests {
        ...
        fn test_simple_quadratic_variants() {
            ...
            let test_cases = vec![
                (
                    F1dim::new(SingleDimFn::new(|x: &TwoFloat| (x - 0.0).powi(2))),
                    0.0.into(),
                    TwoFloat::from_f64(0.0),
                    TwoFloat::from_f64(-2.0),
                    2.0.into(),
                ),
                (
                    F1dim::new(SingleDimFn::new(|x: &TwoFloat| (x - 1.0).powi(2))),
                    1.0.into(),
                    0.0.into(),
                    TwoFloat::from_f64(-1.0),
                    3.0.into(),
                ...
                (
                    F1dim::new(SingleDimFn::new(|x: &TwoFloat| (x - 5.0).powi(2) + 3.0)),
                    5.0.into(),
                    3.0.into(),
                    2.0.into(),
                    8.0.into(),
                ...
                (
                    F1dim::new(SingleDimFn::new(|x: &TwoFloat| {
                ...
                (
                    F1dim::new(SingleDimFn::new(|x: &TwoFloat| (x + 3.0).powi(2))),
                    TwoFloat::from_f64(-3.0),
                    0.0.into(),
                    TwoFloat::from_f64(-5.0),
                    TwoFloat::from_f64(-1.0),
                ),
            ];
            ...
            for (i, (objective, expected_x, expected_f, a, b)) in test_cases.iter().enumerate() {
                ...
                result.xmin.assert_approx_eq(
                    ...
                    format!(
                        ...
                        i, expected_x, result.xmin
                    )
                    .as_str(),
                    "xmin",
                );
                result.fmin.assert_approx_eq(
                    &expected_f,
                    LOOSE_MARGIN.into(),
                    format!(
                        "Test case {}: fmin expected {}, got {}",
                        i, expected_f, result.fmin
                    )
                    .as_str(),
                    "fmin",
                );
        ...
        fn test_higher_order_polynomials() {
            ...
            let quartic = |x: &TwoFloat| (x - 2.0).powi(4) + 1.0;
            let objective = SingleDimFn::new(quartic);
            let mut golden = Golden::new(objective);
            let result = golden
            ...
            let simple_quartic = |x: &TwoFloat| x.powi(4) + 0.5;
            let objective = SingleDimFn::new(simple_quartic);
            let mut golden = Golden::new(objective);
            let result = golden
                .minimize(TwoFloat::from_f64(-2.0), 2.0.into())
                .unwrap();
            assert!(
            ...
            let sixth = |x: &TwoFloat| (x - 1.0).powi(6) + 0.5;
        ...
        fn test_trigonometric_functions() {
            ...
            let trig_identity = |x: &TwoFloat| x.sin().powi(2) + x.cos().powi(2);
        ...
        fn test_exponential_and_logarithmic_functions() {
            ...
            let exp_func = |x: &TwoFloat| TwoFloat::from(E).powf(x - 1.0);
            ...
            let log_func = |x: &TwoFloat| x * x - x.ln(); // Note: minus ln(x)
        ...
        fn test_absolute_value_functions() {
            // |x - 3|, minimum at x = 3
            let abs_func = |x: &TwoFloat| (x - 3.0).abs();
            ...
            let double_abs = |x: &TwoFloat| (x - 2.0).abs() + (x - 4.0).abs();
    ...
    mod convergence_and_precision_tests {
        ...
        fn test_various_tolerance_levels() {
            let f = |x: &TwoFloat| (x - PI).powi(2) + E;
        ...
        fn test_convergence_rates() {
            let f = |x: &TwoFloat| (x - 2.0).powi(4); // Fourth-order function
            let objective = SingleDimFn::new(f);
            let mut golden = Golden::new(objective);
    ...
    mod bracket_and_interval_tests {
        ...
        fn test_various_bracket_sizes() {
            let f = |x: &TwoFloat| (x - 5.0).powi(2) + 1.0;

            let brackets = vec![
                (4.0.into(), TwoFloat::from_f64(6.0)), // Small bracket around minimum
                (0.0.into(), 10.0.into()),             // Medium bracket
                (TwoFloat::from_f64(-100.0), 100.0.into()), // Large bracket
                (4.99.into(), 5.01.into()),            // Very small bracket
                (TwoFloat::from_f64(-1000.0), 1000.0.into()), // Very large bracket
            ];
            ...
            for (a, b) in brackets {
                ...
                result.xmin.assert_approx_eq(
                    &TwoFloat::from(5.0),
                    LOOSE_MARGIN.into(),
                    format!("Bracket [{}, {}]: expected 5.0, got {}", a, b, result.xmin).as_str(),
                    "xmin",
                );
                result.fmin.assert_approx_eq(
                    &twofloat::consts::ONE,
                    LOOSE_MARGIN.into(),
                    format!("Bracket [{}, {}]: expected 1.0, got {}", a, b, result.fmin).as_str(),
                    "fmin",
                );
                assert!(result.converged);
            }
        ...
        fn test_asymmetric_brackets() {
            let f = |x: &TwoFloat| (x - 3.0).powi(2);

            let asymmetric_brackets = vec![
                (0.0.into(), TwoFloat::from_f64(10.0)), // Minimum closer to left
                (TwoFloat::from_f64(-5.0), 4.0.into()), // Minimum closer to right
                (2.9.into(), 20.0.into()),              // Very close to left boundary
                (TwoFloat::from_f64(-50.0), 3.1.into()), // Very close to right boundary
            ];
    ...
    mod edge_cases_and_robustness_tests {
        ...
        fn test_nearly_flat_functions() {
            ...
            let shallow = |x: &TwoFloat| 1e-6 * (x - 5.0).powi(2) + 100.0;
        ...
        fn test_oscillating_functions() {
            ...
            let oscillating = |x: &TwoFloat| (x - 3.0).powi(2) + 0.01 * (20.0 * x).sin();
        ...
        fn test_functions_with_discontinuous_derivatives() {
            ...
            let abs_func = |x: &TwoFloat| (x - 2.0).abs() + 0.1;
            let objective = SingleDimFn::new(abs_func);
            let mut golden = Golden::new(objective);
            let result = golden.minimize(0.0.into(), 4.0.into()).unwrap();
            ...
            result.xmin.assert_approx_eq(
                &TwoFloat::from(2.0),
                VERY_LOOSE_MARGIN.into(),
                "functions_with_discontinuous_derivatives",
                "xmin",
            );
            result.fmin.assert_approx_eq(
                &TwoFloat::from(0.1),
    ...
    mod floating_point_precision_tests {
        ...
        fn test_extreme_function_values() {
            ...
            let large_values = |x: &TwoFloat| 1e12 * (x - 1.0).powi(2) + 1e15;
            ...
            result.xmin.assert_approx_eq(
                &twofloat::consts::ONE,
                LOOSE_MARGIN.into(),
                "extreme_function_values",
                "0",
            );

            // Very small function values
            let small_values = |x: &TwoFloat| 1e-12 * (x - 1.0).powi(2) + 1e-15;
        ...
        fn test_functions_near_zero() {
            ...
            let near_zero = |x: &TwoFloat| (x - 2.0).powi(4) + 1e-16;
    ...
    mod state_consistency_tests {
        ...
        fn test_state_after_successful_optimization() {
            let f = |x: &TwoFloat| (x - 3.0).powi(2) + 5.0;
            let objective = SingleDimFn::new(f);
            let mut golden = Golden::new(objective);
        ...
        fn test_multiple_optimizations() {
            ...
            let result2 = golden
                .minimize(TwoFloat::from_f64(-1.0), 3.0.into())
                .unwrap();
    ...
    mod debug_format_test {
        ...
        fn test_debug_format() {
            ...
            assert!(debug_str.contains("Golden"));
