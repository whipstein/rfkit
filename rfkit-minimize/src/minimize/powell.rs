...
#[derive(Debug, Clone)]
pub struct PowellResult<T: RealScalar> {
    xmin: Points1<T>,
    fmin: T,
    iters: usize,
    fn_evals: usize,
...
impl<T: RealScalar> PowellResult<T> {
...
pub struct Powell<T: RealScalar> {
    xmin: Points1<T>,
    fmin: T,
    f: Box<dyn ObjFn<T>>,
    iters: usize,
...
impl<T: RealScalar> Clone for Powell<T> {
    fn clone(&self) -> Self {
        Powell {
...
impl<T: RealScalar + 'static> Powell<T> {
    ...
    pub fn powell_method(
        ...
        while self.iters < max_iter {
            ...
            let x_extrapolated = Points1::from_shape_fn(n, |i| x[i] * 2.0 - x_old[i]);
            ...
            let temp = (f_old - f_current * 2.0 + f_extrapolated) * 2.0;
    ...
    pub fn powell_method_with_directions(
        ...
        while self.iters < max_iter {
            ...
            for (i, direction) in directions.outer_iter().enumerate() {
                let mut brent = Brent::new(F1dim::new_boxed(self.f.clone()));
                let line_result =
                    brent.line_search(&x, &Points(direction.to_owned()), T::ONE, line_tol, 1000)?;
...
impl<T: RealScalar> fmt::Debug for Powell<T> {
...
mod minimize_powell_tests {
    ...
    mod myfloat_tests {
        ...
        fn test_3d_sphere() {
            ...
            let result = powell
                .minimize(
                    &array![1.0.into(), 2.0.into(), TwoFloat::from(-1.0)].into(),
                    None,
                    None,
                    None,
                    None,
